...
public class HospitalMedCaseServiceBean implements IHospitalMedCaseService {
    ...
    public void refreshReportByPeriod(String aEntranceDate,String aDischargeDate) {
    	...
    	sqlD.append(" delete from AggregateHospitalReport where entrancedate <= (to_date('").append(aDischargeDate).append("','dd.mm.yyyy')-1)") ;
    	sqlD.append(" and (dischargedate >= (to_date('").append(aEntranceDate).append("','dd.mm.yyyy')-1) or dischargerdate is null)") ;
    	theManager.createNativeQuery(sqlD.toString()).executeUpdate() ;
    	StringBuilder sql = new StringBuilder() ;
    	sql.append(" select pat.id as 0patid") ;
    	sql.append(" ,sls.id as 1slsid") ;
    	sql.append(" ,slo.id as 2sloid") ;
    	sql.append(" ,vs.omccode as 3assex") ;
    	sql.append(" ,slo.datestart as 4entrancedate") ;
    	sql.append(" ,coalesce(slo.datefinish,slo.transferdate) as 5dischargedate") ;
    	sql.append(" ,case when a.addressisvillage='1' then '1' else null end as 6isvillage") ;
    	sql.append(" ,case when sls.admissionInHospital_id=1 then '1' else null end as 7isFirstLife") ;
    	sql.append(" ,case when sls.hospitalization_id=1 then '1' else null end as 8isFirstCurrentYear") ;
    	sql.append(" ,case when sls.admissionOrder_id in (2,4,5,6,7,8,9) then '1' else null end as 9isIncompetent") ;
    	sql.append(" ,case when vhr.code='6' then '1' else null end as 10isDeath") ;
    	sql.append(" ,slo.department_id as 11slodepartment") ;
    	sql.append(" ,nextslo.department_id as 12nextslodepartment") ;
    	sql.append(" ,prevslo.department_id as 13prevslodepartment") ;
    	sql.append(" ,list(distinct case when vdrtD.code='4' and vpdD.code='1' then mkbD.code else null end) as 14depDiag") ;
    	sql.append(" ,list(distinct case when vdrt.code='2' then mkb.code else null end) as 15orderDiag") ;
    	sql.append(" ,list(distinct case when vdrt.code='3' and vpd.code='1' then mkb.code else null end) as 16dischargeDiag") ;
    	sql.append(" ,case when count(ahr.id)>0 then '1' else null end as 17cntAggregate") ;
    	sql.append(" ,bf.bedType_id as 18bedType") ;
    	sql.append(" ,bf.bedSubType_id as 19bedSubType") ;
    	sql.append(" ,sls.serviceStream_id as 20serviceStream") ;
    	sql.append(" , case when slo.entranceTime<cast('07:00' as time) then '1' else null as 21entranceTime7") ;
    	sql.append(" , case when slo.entranceTime<cast('09:00' as time) then '1' else null as 22entranceTime9") ;
    	sql.append(" , case when coalesce(slo.dischargerTime,slo.transferTime)<cast('07:00' as time) then '1' else null as 23entranceTime7") ;
    	sql.append(" , case when coalesce(slo.dischargerTime,slo.transferTime)<cast('09:00' as time) then '1' else null as 24entranceTime9") ;
    	sql.append(" , case when sls.emergency='1' then '1' else null end as 25emergency") ;
    	sql.append(" , null as 26operation") ;
    	sql.append(" , vht.code as 27transferLpu") ;
    	...
    	sql.append(" group by vs.omccode,slo.datestart,slo.datefinish,slo.transferdate") ;
