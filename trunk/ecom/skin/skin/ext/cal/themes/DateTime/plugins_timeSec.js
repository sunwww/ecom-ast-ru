...
gbHideBottom=false;
var _is24H=false;	// use 24-hour format or not.
...
var _incSec=5; // incremental time interval in seconds
var _AM="AM", _PM="PM";

var _timeVal,_timeFormat=new RegExp(_is24H?"^([0-1]?[0-9]|2[0-3])[^0-9]+([0-5]?[0-9])[^0-9]+([0-5]?[0-9])$":"^([0]?[1-9]|1[0-2])[^0-9]+([0-5]?[0-9])[^0-9]+([0-5]?[0-9]).*("+_AM+"|"+_PM+")$");
...
function formatTime(str) {
	if (_timeFormat.test(str)==false) { // use current time if str is invalid
		...
		return time2str(h,nd.getMinutes(),nd.getSeconds(),sign);
	} else
		return str;
}
...
function updateTimeStr() {
	...
	_timeVal=time2str(bf.hourF.value,bf.minF.value,bf.secF.value,_is24H?"":bf.ampm.value);
	if (gdSelect[2]>0) {
