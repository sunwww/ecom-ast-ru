 ...
 *
 *  changes (ESinev):
 *       1. ESC - previous value
 *       2. On focus lost - select value (as ENTER)
 *       3. POST - to enable UTF-8 encoding in TOMCAT
 *       4. adapt for IE 5.0 and 5.5 : remove document.createAttribute(...)
 */
...
AjaxJspTag.Autocomplete.prototype = (new AjaxJspTag.Base()).extend({
  ...
  initialize: function(url, options) {
    ...
    this.attachBehaviors(this.options.sourceElem, "keydown", this.onArrowKeys, this);
    this.attachBehaviors(this.options.sourceElem, "blur", this.onLostFocus, this);
    this.attachBehaviors(this.options.sourceElem, "home", this.onHomeKeys, this);
    this.attachBehaviors(this.options.sourceElem, "end", this.onEndKeys, this);
    this.attachBehaviors(this.options.sourceElem, "pagedown", this.onPageDownKeys, this);
    this.attachBehaviors(this.options.sourceElem, "pageup", this.onPageUpKeys, this);
    // ON ESC
    this.oldSourceValue = this.options.sourceElem.value ;
    this.oldTargetValue = this.options.targetElem.value ;
    //
  },
  ...
  onLostFocus: function(e) {
    if ($(this.popupElem).style.visibility == 'visible') {
      ...
      this.executePostFunction();
       ...
       if(this.options.sourceElem.value=='') {
    ...
    if (key == 36) {
      if (this.currentIndex > 0) {
        ...
        this.suggestList[this.currentIndex].scrollIntoView(false);
      	...
      	var lastIndex = this.suggestList.length - 1 ;
      	this.selectSuggest(lastIndex) ;
      }
      //down end
    } else if (key == 35) {
      if(this.currentIndex < this.suggestList.length - 1) {
        ...
        this.suggestList[this.currentIndex].scrollIntoView(false);
      	...
      	this.selectSuggest(0) ;
      }
    }
  },
      
      
  onEndKeys: function(e) {
    var key = 0;
    if (e.keyCode) { key = e.keyCode; }
    else if (typeof(e.which)!= 'undefined') { key = e.which; }
  ...
  autocomplete: function() {
    ...
    for (var i=0; i<items.length; i++) {
      ...
      // in IE 5.0 and 5.5 do not works
  ...
  sourceElemChanged: function(e) {
    ...
    //escape
      ...
      this.hidePopup();
      this.options.targetElem.value = this.oldTargetValue ;
      this.options.sourceElem.value = this.oldSourceValue ;
      Event.stop(e);
