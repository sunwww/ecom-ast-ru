...
<tiles:insert page="/WEB-INF/tiles/mainLayout.jsp" flush="true" >
  ...
  <tiles:put name="body" type="string">
  	<ecom:webQuery name="listArch" nativeSql="select w.id,wp.lastname||' '||wp.firstname||' '||wp.middlename, vwf.name as vwfname, gr.groupName as grgroupName from workfunction wf left join worker w on w.id=wf.worker_id left join Patient wp on wp.id=w.person_id left join WorkFunction gr on gr.id=wf.group_id  left join VocWorkFunction vwf on vwf.id=wf.workFunction_id where w.lpu_id=${param.id} and (wf.archival is null or cast(wf.archival as integer)=0)"/>
