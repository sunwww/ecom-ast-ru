...
<%--    checkFieldUpdate('typeDate','${typeDate}',1) ;
   ...
   function checkFieldUpdate(aField,aValue,aDefaultValue) {
    ...
    <msh:sectionContent>
    <ecom:webQuery name="journal_priem" nativeSql="select vbst.name as vbstname,vss.name as vssname
...
-count(distinct case when sls.hotelServices='1' and (slo.datefinish = to_date('${dateBegin}','dd.mm.yyyy') and slo.dischargetime>=cast('09:00' as time)
or slo.datefinish = to_date('${dateNext}','dd.mm.yyyy') and cast('09:00' as time)>slo.dischargetime) then slo.id else null end)
-count(distinct case when sls.hotelServices='1' and (slo.transferdate = to_date('${dateBegin}','dd.mm.yyyy') and slo.transfertime>=cast('09:00' as time)
or slo.transferdate = to_date('${dateNext}','dd.mm.yyyy') and cast('09:00' as time)>slo.transfertime) then slo.id else null end)
...
group by bf.bedsubtype_id,vbst.name,bf.serviceStream_id,vss.name
      " />
    <msh:table name="journal_priem" viewUrl="entityShortView-stac_ssl.do" action="entityParentView-stac_ssl.do" idField="1" guid="b621e361-1e0b-4ebd-9f58-b7d919b45bd6">
      ...
      <msh:tableColumn columnName="профиль коек" property="1" />
      <msh:tableColumn columnName="поток обслуживания" property="2" />
      <msh:tableColumn columnName="Состоит на начало истекших суток" property="3" />
      <msh:tableColumn columnName="поступило всего" property="4" />
      <msh:tableColumn columnName="в т.ч. из дневного стационар" property="5" />
      <msh:tableColumn columnName="с.ж" property="6" />
      <msh:tableColumn columnName="до 17 лет" property="7" />
      <msh:tableColumn columnName="60 лет и старше" property="8" />
      <msh:tableColumn columnName="переведено из других отд" property="9" />
      <msh:tableColumn columnName="переведено в другие отд" property="10" />
      <msh:tableColumn columnName="выписано" property="11" />
      <msh:tableColumn columnName="в другие стационары" property="12" />
      <msh:tableColumn columnName="в круглосуточный стационар" property="13" />
      <msh:tableColumn columnName="в дневной стационар" property="14" />
      <msh:tableColumn columnName="умерло" property="15" />
      <msh:tableColumn columnName="состоит всего" property="16" />
      <msh:tableColumn columnName="состоит матерей" property="17" />
    </msh:table>
    </msh:sectionContent>
    </msh:section>
    <%
...
order by pat.lastname,pat.firstname,pat.middlename
    ...
    <msh:sectionTitle>007/у-02 форма</msh:sectionTitle>
    <msh:sectionContent>
    <ecom:webQuery name="journal_priem" nativeSql="select '${dateBegin}'||':'||lpu.id,lpu.name,list(vbst.name) as vbstname,list(vss.name) as vssname
,count(distinct case when (slo.datestart = to_date('${dateBegin}','dd.mm.yyyy') and cast('09:00:00' as time)>slo.entrancetime
or to_date('${dateBegin}','dd.mm.yyyy')>slo.datestart)
 and (to_date('${dateBegin}','dd.mm.yyyy')>slo.datestart or
slo.datestart = to_date('${dateNext}','dd.mm.yyyy') and cast('09:00' as time)>slo.entrancetime)
...
group by lpu.id,lpu.name
      " />
    <msh:table name="journal_priem" viewUrl="stac_report_007.do?short=Short" action="stac_report_007.do" idField="1" guid="b621e361-1e0b-4ebd-9f58-b7d919b45bd6">
      <msh:tableColumn columnName="#" property="sn" />
      ...
      <msh:tableColumn columnName="с.ж" property="8" />
      <msh:tableColumn columnName="до 17 лет" property="9" />
      <msh:tableColumn columnName="60 лет и старше" property="10" />
      <msh:tableColumn columnName="переведено из других отд" property="11" />
      <msh:tableColumn columnName="переведено в другие отд" property="12" />
      <msh:tableColumn columnName="выписано" property="13" />
      <msh:tableColumn columnName="в другие стационары" property="14" />
      <msh:tableColumn columnName="в круглосуточный стационар" property="15" />
      <msh:tableColumn columnName="в дневной стационар" property="16" />
      <msh:tableColumn columnName="умерло" property="17" />
      <msh:tableColumn columnName="состоит всего" property="18" />
      <msh:tableColumn columnName="состоит матерей" property="19" />
    </msh:table>
    </msh:sectionContent>
    </msh:section>
  ...
  <script type="text/javascript">
  if ('${param.id}'!='') {
  	var id='${param.id}'.split(':') ;
  	departmentAutocomplete.setVocId(id[1]) ;
  	$('dateBegin').value=id[0] ;
  	 checkFieldUpdate('typeView','1',1) ;
  }
  </script>
  </tiles:put>
</tiles:insert>
