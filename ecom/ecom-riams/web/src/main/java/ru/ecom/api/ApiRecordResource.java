    ...
    private JSONObject makeRecordOrAnnul(HttpServletRequest aRequest, JSONObject root) {
        try {
            ...
            if (doctorPromedCode != null) { //Если получаем ИД врача в промеде - находим ИД самостоятельно
                ...
                if (calendarId == null) {
                    return ApiRecordUtil.getErrorJsonObj("Не найдено свободное время", "BUSY_CALENDARTIME");
                }
            }
            if (calendarId == null) {
                return ApiRecordUtil.getErrorJsonObj("Не указано время записи", "NO_CALENDARTIME");
            }
            ...
            if (requestId != null)
                LOG.info("Запрос №" + requestId + " (makeRecordOrAnnul) обработан, вот ответ: " + list);
