...
public class Expert2ServiceJs {
    ...
    public String fixSomeErrors(Long aListEntryId, String aErrorCode, HttpServletRequest aRequest) throws NamingException, SQLException {
        if ("223".equals(aErrorCode)) {
            ...
            service.executeUpdateNativeSql("update e2entry set smocode = '" + insuranceCode + "' where listentry_id=" + aListEntryId + " and (smocode is null or smocode='')" +
                    " and (isDeleted is null or e.isDeleted='0') " +
                    " and (isForeign is null or isForeign ");
            return "Страховая компания заменена на 300002";
        } else {
    ...
    public String replaceFieldByError(Long aEntryListId, String aErrorCode, String aFieldName, String aOldValue, String aNewValue, HttpServletRequest aRequest) throws NamingException {
        if (StringUtil.isNullOrEmpty(aFieldName) || StringUtil.isNullOrEmpty(aNewValue) || StringUtil.isNullOrEmpty(aOldValue)) {
            return "неуспешно!";
        }
    ...
    public boolean saveBillDateAndNumber(Long aListEntryId, String aType, String aServiceStream, String aOldBillNumber, String aOldBillDate, String aBillNumber
        ...
        if (aServiceStream == null || aServiceStream.trim().equals("")) {
            return false;
        }
        if (StringUtil.isNullOrEmpty(aBillNumber)) { //Удалить информацию о номере счета.
            if (aOldBillDate == null || aOldBillDate.equals("")) {
                return false;
            }
            if (aOldBillNumber == null || aOldBillNumber.equals("")) {
                return false;
            }
        ...
        if (!StringUtil.isNullOrEmpty(aOldBillDate)) {
            sql.append(" and billDate=to_date('").append(aOldBillDate).append("','dd.MM.yyyy')");
        } else {
