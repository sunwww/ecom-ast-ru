...
public class DisabilitySign {
    ...
    public String exportDisabilityDocument(@Context HttpServletRequest aRequest, @WebParam(name="token") String aToken,
        ...
        String close ="select \n" +
                ...
                "where \n" +
                "ds.id = (select max(id) from  disabilitysign where disabilitydocumentid_id ="+disDoc+" and noactual=false and code='close')\n" +
                "and dr.dateto =  (select max(dateto) from disabilityrecord  where disabilitydocument_id = "+disDoc+")";
