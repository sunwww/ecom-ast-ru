...
public class OncologyServiceJs {
    ...
    public String insertDirection(String json, String id, String mkb, HttpServletRequest aRequest) throws JSONException, NamingException {
        ...
        for (int i = 0; i < arr.length(); i++) {
            ...
            if (id.equals("")) {
                ...
                service.executeUpdateNativeSql("update oncologycase set mkb='" + mkb + "' where id=" + id); //обновить МКБ онк. формы
    ...
    public String getDates(String caseId, HttpServletRequest aRequest) throws NamingException {
        ...
        if (!list.isEmpty()) {
            for (WebQueryResult wqr : list)
                res.append(wqr.get1()).append("#").append(wqr.get2()).append("#").append(wqr.get3()).append("#").append(wqr.get4());
        } else res.append("##");
        return res.toString();
    }
    ...
    public String checkDiagnosisOnkoForm(Long slsId, String concludingMkb, HttpServletRequest aRequest) throws NamingException {
        ...
        try {
            ...
            sql.append("select c.mkb||' '||mkb.name||' ('||prior.name||' '||reg.name||')',c.id from oncologycase c")
                    ...
                    .append(" where c.medcase_id=").append(slsId);
            ...
            if (list.isEmpty())
                list = service.executeNativeSql(sql.toString() + " and prior.code='1' and reg.code='4' ");  //основной клинический
            ...
            if (!flag && !list.isEmpty()) {
                if (!concludingMkb.startsWith("C") && ds.startsWith("C"))
                    res = "Внимание! В случае установки этого диагноза неактуальная онкологическая форма будет удалена. Вы уверены?#" + cId;
                else if (!concludingMkb.startsWith("C") && !ds.startsWith("C"))
                    res = "";  //пока так: подозрение на подозрение - менять форму не нужно
