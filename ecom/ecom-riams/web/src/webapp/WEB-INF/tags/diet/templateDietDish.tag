...
<%@ attribute name="record" required="true"
              description="Тип записи: 1-в существующее меню, 2-в новое меню-заказ, 3-в новый шаблон меню" %>
<%@ attribute name="parentId" required="false"
              description="ИД родителя: при record=1-ИД шаблона меню, а 2,3 - ИД диеты берется из шаблона" %>
    ...
    <div class='rootPane'>
        ...
        <form>
            <msh:panel>
                ...
                <msh:row>
                    <msh:autoComplete label="Шаблон меню-раскладки" property="${name}templateMenu"
                                      vocName="vocMenuByDiet" horizontalFill="true"
                                      parentAutocomplete="${name}templateDiet" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete label="Шаблон приема пищи" property="${name}templateChildMenu"
                                      vocName="vocChildMenu" horizontalFill="true"
                                      parentAutocomplete="${name}templateMenu" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
