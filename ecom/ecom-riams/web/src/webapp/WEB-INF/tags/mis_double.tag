...
<div id='${name}DoubleDialog' class='dialog'>
    ...
    <div class='rootPane'>
        ...
        <form action="javascript:">
            ...
            <msh:row>
                <td colspan="6">
                    <input type="button" name='${name}DoubleSave' style="display: none;" id='${name}DoubleSave'
                           value='Продолжить сохранение' onclick='javascript:next${name}Double()'/>
                    <input type="button" name='${name}DoubleCancel' id='${name}DoubleCancel'
                           value='Вернуться к редактированию' onclick='javascript:cancel${name}Double()'/>
                </td>
            </msh:row>
        </form>
