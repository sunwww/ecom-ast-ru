    ...
    <div class='rootPane'>
        ...
        <form action="javascript:void(0) ;" id="formServiceAndDiagnosis">
            <table width="100%" cellspacing="0" cellpadding="4">
                ...
                <msh:row>
                    <msh:autoComplete label="МКБ" property="${name}MkbId" vocName="vocIdc10" size="50"
                                      horizontalFill="true" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete property="${name}RegistrationType" label="Тип регистрации" horizontalFill="true"
                                      fieldColSpan="1" vocName="vocDiagnosisRegistrationType"/>
                </msh:row><msh:row>

            </msh:row><msh:row>
                <msh:autoComplete vocName="vocE2FondV027" property="${name}IllnesPrimary" label="Характер заболевания"
                                  horizontalFill="true" fieldColSpan="3"/>
            </msh:row><msh:row>
                <msh:autoComplete vocName="vocPriorityDiagnosis" property="${name}Priority" label="Приоритет"
                                  horizontalFill="true"/>
            </msh:row>
                <tr>
                ...
                <tr>
                    <td colspan="4"><label><input type="checkbox" name="${name}IsMainMkb" id="${name}IsMainMkb"/>Диагноз
                        является главным в случае?</label></td>
                </tr>
                <tr>
                    <td colspan="4" align="right" width="100"><b><u>Добавить услугу</u></b></td>
                </tr>
                <msh:autoComplete property="${name}MedService" vocName="vocMedService" size="50" label="Добавить услугу"
                                  horizontalFill="true" fieldColSpan="3"/>
                <tr>
                    <msh:textField property="${name}MedServiceDate" label="Дата услуги"/>
                </tr>
                <tr>
                    <td colspan="4"><label><input type="checkbox" name="${name}IsMainService"
                                                  id="${name}IsMainService"/>Услуга является главной в случае?</label>
                    </td>
                </tr>

                <tr>
                    <td><input type="button" value='Создать' id="${name}Save"
                               onclick='javascript:save${name}Service()'/></td>
                    <td><input type="button" value='Закрыть' id="${name}Cancel"
                               onclick='javascript:cancel${name}Service()'/></td>
                </tr>
            </table>
        </form>
