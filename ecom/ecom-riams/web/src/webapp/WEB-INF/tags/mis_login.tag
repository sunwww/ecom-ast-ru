    ...
    <div class='rootPane'>
        ...
        <form action="javascript:save${name}Password()" target="_blank">
            <msh:panel styleId="panel${name}">
                <msh:row>
                    <msh:textField property="${name}Login" label="Введите логин" fieldColSpan="3"
                                   horizontalFill="true"/>
                    <msh:textField property="password" label="Введите пароль" fieldColSpan="3" horizontalFill="true"
            ...
            <div id='div${name}Button'>
                <msh:row>
                    <td colspan="6">
                        <input type="button" value='Ok' id="${name}Ok"
                               onclick="this.form.${name}Login.name='username';this.form.action='ecom_loginSave.do';this.form.submit();cancel${name}Autorization();this.form.username.name='${name}Login';"/>
                        <input type="button" value='Отменить' id="${name}Cancel"
                               onclick="cancel${name}Autorization();"/>
                    </td>
                </msh:row>
                <msh:row>
                    <td colspan="4">ПОСЛЕ ВВОДА ЛОГИНА И ПАРОЛЯ НЕ ЗАБУДЬТЕ ЗАКРЫТЬ НОВУЮ ВКЛАДКУ И СОХРАНИТЬ
                        ДАННЫЕ!!!!!
                    </td>
                </msh:row>
            </div>
