    ...
    <div class='rootPane'>
        <form name="frm${name}TemplateProt">
            <table>
                <tr>
                    <td colspan="3">
                        <a href="javascript:void(0)" onclick="load${name}ListProtocols('temp')" id="temp${name}A">Шаблоны
                            все</a>
                        <a href="javascript:void(0)" onclick="load${name}ListProtocols('my')" id="my${name}A">Свои
                            шаблоны</a>
                        <a href="javascript:void(0)" onclick="load${name}ListProtocols('mydiary')" id="mydiary${name}A">Список
                            своих заключений по пациенту</a>
                        <a href="javascript:void(0)" onclick="load${name}ListProtocols('polyc')" id="polyc${name}A">Список
                            заключения по пациенту</a>
                        <a href="javascript:void(0)" onclick="load${name}ListProtocols('hospit')" id="hospit${name}A">Госпитализация
                            (осмотры врачей)</a>
                        <a href="javascript:void(0)" onclick="load${name}ListProtocols('disch')" id="disch${name}A">Госпитализация
                            (выписки)</a>

                    </td>
                </tr>
                <msh:row styleId="noswod">
                    <td class="label" title="Поиск по показаниям поступления (${name}typeReestr)" colspan="1"><label
                            for="${name}typeReestrName" id="${name}typeReestrLabel">Отобразить:</label></td>
                ...
                <tr>
                    <td colspan="3">
                        ...
                        <msh:row>
                    <td colspan="3">
                        <input type="button" name="buttonTempProtOk" id='buttonTempProtOk' value='Добавить в протокол'
                               onclick='save${name}TemplateProtocol()'/>
    ...
    function save${name}IntakeInfo() {
        ...
        for (var ind = 0; ind < fldJson.params.length; ind++) {
            ...
            if (+par.type == 2) {
                ...
                if (val.length > 0) {
                    val += ",";
                    par.valueVoc = par.valueVoc.substring(2);
                }
            } else if (+par.type == 7) {
                if (+val < 1) {
                    val = '0';
    ...
    function load${name}ListProtocols(aType, aParent, aSearchText) {
        ...
        if (+aParent > 0 || (+aParent == -1) || aParent.length > 0) {
            ...
            if (aType == 'temp') {
                ...
                isR = document.forms['frm${name}TemplateProt'].${name}typeReestr[0].checked;
            }
        }
