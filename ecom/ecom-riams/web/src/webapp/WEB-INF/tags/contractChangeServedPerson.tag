...
<form action="javascript:save${name}ContractPerson()">
    <msh:panel styleId="panel${name}" >
    	<msh:row>
    	<msh:autoComplete label="Обслуживаемая персона" property="new${name}ServedPerson" vocName="contractPerson" size="100"/>
    	</msh:row>
        ...
        <msh:row>
            <td colspan="6">
                <input type="button" value='Изменить обслуживаему персону' id="${name}Ok"  onclick="javascript:save${name}ContractPerson()"/>
...
<script type="text/javascript">
     ...
     function save${name}ContractPerson() {
         if (!confirm("Вы уверены что хотите изменить обслуживаемую персону?")){return;}
