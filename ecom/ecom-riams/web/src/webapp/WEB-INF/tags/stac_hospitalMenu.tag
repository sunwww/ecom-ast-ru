...
<msh:sideMenu title="Показать">
    ...
    <msh:sideLink styleId="viewShort"
                  action="/javascript:getDefinition('expert_listBySls.do?short=Short&id=${param.id}',null)"
    ...
    <msh:sideLink roles="/Policy/Mis/MedCase/QualityEstimationCard/View" name="Экспертные карты" params="id"
                  action="/entityParentList-expert_card"/>
    ...
    <msh:sideLink styleId="viewShort" action="/javascript:viewOtherVisitsByPatient('.do')" name='ВИЗИТЫ'
                  title="Просмотр визитов по пациенту" key="ALT+4" params="" roles="/Policy/Mis/MedCase/Visit/View"/>
    <msh:sideLink styleId="viewShort" action="/javascript:viewOtherDiagnosisByPatient('.do')" name='ДИАГНОЗЫ'
                  title="Просмотр диагнозов по пациенту" key="ALT+5" params=""
                  roles="/Policy/Mis/MedCase/Diagnosis/View,/Policy/Mis/MedCase/Ssl/Diagnosis/View"/>
    <msh:sideLink styleId="viewShort" action="/javascript:viewOtherExtMedserviceByPatient('.do')"
                  name='Внешние лаб. исследования' title="Просмотр внешних лабораторных данных по пациенту" key="ALT+5"
                  params="" roles="/Policy/Mis/MedCase/Document/External/Medservice/View"/>
    <msh:sideLink styleId="viewShort"
                  action="/javascript:getDefinition('js-stac_sslAllInfo-list_fond_direct_by_sls.do?short=Short&id=${param.id}','.do')"
                  name='Направления из фонда' title="Просмотр прикрепленных направлений из фонда"
                  roles=""/>
    <msh:sideLink styleId="viewShort"
                  action="/javascript:getDefinition('js-stac_ssl-cost_case.do?short=Short&id=${param.id}','.do')"
    ...
    <msh:sideLink roles="/Policy/Mis/Prescription/Prescript/View" name="Лист назначений"
                  ...
                  title='Показать/добавить лист назначений'
    />
    <msh:sideLink roles="/Policy/Mis/Prescription/Prescript/View" name="Сводный лист назначений" params=""
                  action="/javascript:getDefinition('entityParentList-pres_prescriptList.do?short=Short&id=${param.id}')"
                  title="Показать все назначения СЛС"/>
    <msh:sideLink roles="/Policy/Mis/MedCase/Stac/Ssl/SurOper/ShowSls,/Policy/Mis/MedCase/Stac/Ssl/SurOper/View"
                  name="Операции"
                  styleId="viewShort"
                  action="/javascript:getDefinition('entityParentList-stac_surOperation.do?short=Short&id=${param.id}','.do')"
                  title='Операции'
    ...
    <msh:sideLink roles="/Policy/Mis/MedCase/Protocol/View,/Policy/Mis/MedCase/Stac/Ssl/Protocol/View"
                  name="Дневник специалиста ПРИЕМНОГО ОТДЕЛЕНИЯ"
    ...
    <msh:sideLink styleId="stac_protocol" params="id"
                  roles="/Policy/Mis/MedCase/Protocol/View,/Policy/Mis/MedCase/Stac/Ssl/Protocol/View"
    ...
    <msh:sideLink roles="/Policy/Mis/Pregnancy/BirthNosologyCard/View" name="Карта по нозологиям" params=""
                  styleId="viewShort" action="/javascript:showbirthNosologyCard(${param.id},null,0)"
                  title="Карта по нозологиям"
    />
    <msh:sideLink roles="/Policy/Mis/Pregnancy/BirthNosologyCard/View" name="Карта по нозологиям: отмеченные" params=""
                  styleId="viewShort" action="/javascript:showOnlyCheckedbirthNosologyCard(${param.id})"
                  title="Карта по нозологиям: отмеченные"
    />
</msh:sideMenu>
...
<msh:sideMenu title="Печать">
    <msh:sideLink roles="/Policy/Mis/MedCase/Stac/Ssl/PrintStatCard3"
                  ...
                  action='/javascript:initSelectPrinter("print-statcard.do?m=printStatCardInfo&s=HospitalPrintService&id=${param.id}",1)'
                  title='Печать истории болезни'
    />
    <msh:sideLink roles="/Policy/Mis/MedCase/Stac/Ssl/PrintStatCard"
                  name="Печать стат. карты в формате А4"
                  action='/javascript:initSelectPrinter("print-statcard_four.do?m=printStatCardInfo&s=HospitalPrintService&id=${param.id}",1)'
                  title='Печать истории болезни формат А4'
    />
    <msh:sideLink roles="/Policy/Mis/MedCase/Stac/Ssl/AddressSheetByHospital"
                  name="Печать адресных листков (прибытия и убытия)"
                  action='/javascript:initSelectPrinter("print-listAddressHospital.do?m=printAddressSheetByHospital&s=HospitalPrintService&id=${param.id}",1)'
                  title='Печать адресных листков'
    />
    <msh:sideLink roles="/Policy/Mis/MedCase/Stac/Ssl/PrintStatCard066"
                  name="Печать стат.карты выбывшего из стационара"
                  action='/javascript:initSelectPrinter("print-066.do?m=printStatCardInfo&s=HospitalPrintService&id=${param.id}",1)'
                  title='Печать стат.карты выбывшего из стационара'
    />
    <msh:sideLink roles="/Policy/Mis/MedCase/Stac/Ssl/PrintStatCard066All"
                  name="Печать стат.карты выбывшего из стационара"
                  action='/javascript:initSelectPrinter("print-066_all.do?m=printStatCardInfo&s=HospitalPrintService&check=1&id=${param.id}",1)'
                  title='Печать стат.карты выбывшего из стационара'
    />
    <msh:sideLink roles="/Policy/Mis/MedCase/Stac/Ssl/PrintStatCard033"
                  ...
                  action='/javascript:initSelectPrinter("print-003.do?m=printStatCardInfo&s=HospitalPrintService&id=${param.id}",1)'
                  title='Печать стат.карты выбывшего из стационара'
    />
    <msh:sideLink roles="/Policy/Mis/MedCase/Stac/Ssl/PrintDischarge"
                  name="Печать выписки"
                  action='/javascript:initSelectPrinter("print-discharge_hospital.do?m=printBilling&s=HospitalPrintService&id=${param.id}",1)'
                  title='Печать выписки'
    />
    <msh:sideLink roles="/Policy/Mis/MedCase/Stac/Ssl/PrintDischarge"
                  name="Печать экс. карты"
                  action='/javascript:initSelectPrinter("print-expert_card_empty.do?m=printBilling&s=HospitalPrintService&id=${param.id}",1)'
                  title='Печать экс. карты'
    />
...
<msh:sideMenu>
    ...
    <msh:sideLink styleId="viewShort"
                  action="/javascript:getDefinition('entityParentList-stac_deathCase.do?short=Short&id=${param.id}','.do')"
                  name='Случай смерти' title="Просмотр случая смерти"
    ...
    <msh:sideLink roles="/Policy/Mis/MedCase/Stac/Ssl/Planning/View" name="Направления на госпитализацию"
                  params="id" action='/entityParentList-smo_planHospitalByHosp'
                  title='Список направлений на госпитализацию'
...
<msh:sideMenu title="Администрирование">
    ...
    <tags:mis_changeServiceStream name="CSS" title="Изменить поток обслуживания"
                                  roles="/Policy/Mis/MedCase/Stac/Ssl/ChangeServiceStream"/>
...
<msh:sideMenu title="Дополнительно">
    <msh:sideLink action="/stac_sslList.do?sslid=${param.id}" name="⇧Все госпитализации пациента"
                  title="Все госпитализации пациента"/>
    <msh:sideLink action="/mis_patients" name="Новая госпитализация"
                  roles="/Mis/MainMenu/Patient,/Policy/Mis/MedCase/Stac/Ssl/Admission/Create"/>
    <msh:sideLink action="/javascript:makeTimurHappy()" name="Расчитать вероятность диабета"
                  roles="/Policy/Mis/Journal/CheckDiabetes"/>
</msh:sideMenu>

<msh:sideMenu title="Добавить">
    <msh:sideLink action="/entityParentPrepareCreate-stac_disabilityCase" name="Нетрудоспособность" params="id"
                  title="Нетрудоспособность" roles="/Policy/Mis/MedCase/Stac/Ssl//Disability/Disability/Create"/>
</msh:sideMenu>
                ...
                callback: function (res) {
                    if (res == true) {
                        ...
                        showToastMessage("Невозможно удалить данные! " +
                            "Удалить могут только: КЭО или в течение заданного в настройках периода пользователь с соответствующей ролью (зав. отделения)" +
