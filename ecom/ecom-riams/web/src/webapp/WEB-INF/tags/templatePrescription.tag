...
<%@ attribute name="record" required="true"
              description="Тип записи: 1-в существующий лист назначений, 2-в новый лист назначений" %>
<%@ attribute name="parentId" required="false"
              description="ИД родителя: при record=1-ИД листа назначений, а 2-ИД СМО" %>
...
<div id='${name}templatePrescriptionDialog' class='dialog'>
    ...
    <div class='rootPane'>
        ...
        <form>
            <msh:panel>
                <msh:row>
                    <msh:autoComplete label="Категория" property="${name}templateCategory" vocName="vocTemplateCategory"
                                      horizontalFill="true"/>
                </msh:row>
                <msh:autoComplete label="Шаблон" property="${name}templatePrescription"
                                  vocName="vocTemplatePrescription" horizontalFill="true"
                                  parentAutocomplete="${name}templateCategory"/>
                <msh:row>
                    <msh:textArea label='Описание назначения' property="${name}decriptionTextPrescription"
                                  horizontalFill="true"/>
                </msh:row>
            </msh:panel>
            <msh:row>
