    ...
    <div class='rootPane'>
        ...
        <form action="javascript:save${name}ServiceFind()">
            <msh:panel styleId="panel${name}">
                ...
                <msh:row>
                    <msh:autoComplete property="${name}PriceList" label="Прейскурант" vocName="priceList"
            ...
            <div id='div${name}Button'>
                <msh:row>
                    <td colspan="6">
                        <input type="button" value='Поиск' id="${name}Ok"
                               onclick="javascript:save${name}ServiceFind()"/>
                        <msh:ifInRole roles="/Policy/Mis/MedService/AddFromPriceList">
                            <input name="find${name}Button" type="button" value='Поиск по прейскуранту'
                                   onclick="javascript:find${name}ServiceFindByPriceList()"/>
                        </msh:ifInRole>
                        <input type="button" value='Отменить' id="${name}Cancel"
                               onclick='javascript:cancel${name}ServiceFind()'/>
                    </td>
                </msh:row>
            </div>
        </form>
