    ...
    <div class='rootPane'>
        ...
        <form action="javascript:void(0) ;">
            <msh:panel>
                ...
                <msh:row>
                    <msh:autoComplete label="Причина закрытия" property="${name}Reason"
                                      vocName="vocDisabilityDocumentCloseReason" horizontalFill="true"
                                      fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:textField labelColSpan="3" property="${name}OtherCloseDate"
                                   label="Иная дата закрытия для причин 32, 33, 34, 36"/>
                </msh:row>
            </msh:panel>
            <msh:row>
                <td colspan="6">
                    <input type="button" id='${name}buttonAddressOk' value='OK'
                           onclick="javascript:save${name}CloseDocument()"/>
