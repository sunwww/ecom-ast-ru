    ...
    <div class='rootPane'>
        ...
        <form action="javascript:save${name}ServiceWorkFunction()">
            <msh:panel styleId="panel${name}">
                ...
                <msh:row>
                    <msh:autoComplete size="100" vocName="vocWorkFunction" property="${name}vocWorkFunction"
                                      label="Раб. функция" horizontalFill="true" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete size="100" vocName="serviceWorkFunction" property="${name}workFunction"
                                      label="Специалист (группа)" horizontalFill="true" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete size="100" vocName="lpu" property="${name}lpu" label="ЛПУ" horizontalFill="true"
                                      fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete size="100" vocName="vocBedType" property="${name}bedType" label="Профиль коек"
                                      horizontalFill="true" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete size="100" vocName="vocBedSubType" property="${name}bedSubType" label="Тип коек"
                                      horizontalFill="true" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete size="100" vocName="vocRoomType" property="${name}roomType" label="Уровень палат"
                                      horizontalFill="true" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete size="100" property="${name}prescriptType" vocName="vocPrescriptType"
                                      fieldColSpan="3" horizontalFill="true" label="Тип назначения"/>
                </msh:row>
                <msh:row>
                    <msh:checkBox property="${name}noActiveByPrescript" fieldColSpan="3"
                                  label="Не отображать в назначениях"/>
                </msh:row>
            </msh:panel>
            <msh:row>
                <td colspan="6">
            ...
            <div id='div${name}Button'>
                <msh:row>
                    <td colspan="6">
                        <input type="button" value='Добавить прикрепление' id="${name}Ok"
                               onclick="javascript:save${name}ServiceWorkFunction()"/>
                        <input type="button" value='Отменить' id="${name}Cancel"
                               onclick='javascript:cancel${name}ServiceWorkFunction()'/>
                    </td>
                </msh:row>
            </div>
        </form>
