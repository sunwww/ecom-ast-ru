...
<%@ attribute name="alreadyUser" required="true" description="Пользователь или персона" %>
<!-- true - пользователь, false - персона -->
...
<div id='${name}CloseDocumentDialog' class='dialog'>
    ...
    <h4>При создании через <i>Настройки\Пользователи</i> обязательные поля <u>ЛПУ, рабочая функция</u> и/или
        <u>пароль</u>.</h4>
    <h4>(можно просто сбросить пароль/создать рабочую функцию/всё вместе)</h4>
    <h4>При создании через <i>Персону</i> обязательные поля <u>Логин, ЛПУ, рабочая функция</u> и/или <u>пароль</u> и/или
        <u>роли</u>.</h4>
    ...
    <div>
        <form id="${name}">
            <msh:panel>
                ...
                <msh:row>
                    <msh:autoComplete property="${name}userCopy" vocName="secUser"
                                      label="Копировать роли у пользователя" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <br><label><input name="${name}isCovid" id="${name}isCovid" type="checkbox"
                                      onclick='javascript:setCovidOrNot${name}();'/>Инфекционное?</label><br>
                </msh:row>
                <msh:row>
                    <msh:comboBox size='300' horizontalFill="true" property='${name}vocLpuOtd' vocName="vocLpuOtd"
                                  label='Отделение:' parentId="1"/>
                </msh:row>
                <msh:row>
                ...
                <msh:row>
                    ...
                    <table width="100%" cellspacing="10" cellpadding="10">
                        <tr>
                            <td align="center"><label><input name="${name}rad" type="radio"
                                                             onclick='javascript:setText${name}("1");'/>1</label></td>
                            <td align="center"><label><input name="${name}rad" type="radio"
                                                             onclick='javascript:setText${name}("Covid-19");'/>Covid-19</label>
                            </td>
                        </tr>
                        <tr>
                            <td></td>
    ...
    <div>
        <table width="100%" cellspacing="10" cellpadding="10">
            <tr>
                <td align="center"><input type="button" value='Выполнить' id="${name}Add"
                                          onclick='javascript:make${name}()'/></td>
            </tr>
            <tr>
                <td></td>
            </tr>
            <tr>
                <td align="right"><input type="button" style="font-weight:bold" id="${name}203" value='Отменить'
