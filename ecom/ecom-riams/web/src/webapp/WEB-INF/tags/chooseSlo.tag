...
<div id='${name}CloseDocumentDialog' class='dialog'>
    ...
    <div>
        <table width="100%" cellspacing="10" cellpadding="10">
            <tr><td></td></tr>
            <tr>
                <td align="right"><input type="button"  style="font-weight:bold" id="${name}" value='Отменить' id="${name}Cancel" onclick='the${name}CloseDocumentDialog.hide()'/></td>
            </tr>
        </table>
    </div>
</div>
<script type="text/javascript">
    ...
    function show${name}() {
        ...
        table.innerHTML="";
        PregnancyService.getAllSLOFromSLSBySLO(${sloId},{callback: function(res) {
                if (res!=null && res!='[]') {
