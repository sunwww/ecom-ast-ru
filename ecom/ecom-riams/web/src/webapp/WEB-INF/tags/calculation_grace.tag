        ...
        function save${name}NewCalculation() {
            ...
            if(formula${name}.innerHTML!=''){
                ...
                if (record!=null) {
                    ...
                    record.value += "Шкала GRACE: " + temp + ".\n";
                    record.value += "Острый коронарный синдром с подъемом сегмента ST:\n";
                    if (temp < 126) record.value += '- риск внутригоспитальной  летальности: низкий\t< 2%';
                    if (temp < 100) record.value += '\n-в течение 6 месяцев: низкий\t< 4.5%';
                    if (temp >= 126 && temp<=154) record.value += '- риск внутригоспитальной  летальности: cредний\t< 2 - 5%';
                    if (temp >= 100 && temp<=127)  record.value += '\n-в течение 6 месяцев: cредний\t< 4.5 - 11%';
                    if (temp > 154) record.value += '- риск внутригоспитальной  летальности: высокий\t> 5%';
                    if (temp > 127) record.value += '\n-в течение 6 месяцев: высокий\t> 11%';

                    record.value += "\nОстрый коронарный синдром без подъема ST:\n";
                    if (temp < 109) record.value += '- риск внутригоспитальной  летальности: низкий\t< 1%';
                    if (temp < 89) record.value += '\n-в течение 6 месяцев: низкий\t< 3%';
                    if (temp >= 109 && temp<=140) record.value += '- риск внутригоспитальной  летальности: cредний\t< 1 - 3%';
                    if (temp >= 89 && temp<=118)  record.value += '\n-в течение 6 месяцев: cредний\t< 3 - 8%';
                    if (temp > 140) record.value += temp + '- риск внутригоспитальной  летальности: высокий\t> 3%';
                    if (temp > 118) record.value += '\n-в течение 6 месяцев: высокий\t> 8%';
