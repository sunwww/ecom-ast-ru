...
<tiles:insert page="/WEB-INF/tiles/mainLayout.jsp" flush="true">
    ...
    <tiles:put name="body" type="string">
        <% String orderByType = ActionUtil.updateParameter("BloodReport", "orderByType", "3", request); %>
        <msh:form action="/dis_documentClose.do" defaultField="beginDate" disableFormDataConfirm="true" method="GET">
            <msh:panel>
                ...
                <msh:row>
                    <td class="label" title="Статус документа  (typeDocument)" colspan="1"><label for="typeDateName"
                                                                                                  id="typeDateLabel">Статус
                        документа:</label></td>
                    <td onclick="this.childNodes[1].checked='checked';">
                ...
                <msh:row>
                    <td class="label" title="ЛПУ выдачи  (typeLpu)" colspan="1"><label for="typeLpuName"
                                                                                       id="typeLpuLabel">ЛПУ
                        выдачи:</label></td>
                    <td onclick="this.childNodes[1].checked='checked';">
                ...
                <msh:row>
                    <td class="label" title="Искать в документах  (noActualityt)" colspan="1"><label
                            for="noActualityName" id="noActualityLabel">Искать в документах:</label></td>
                    <td onclick="this.childNodes[1].checked='checked';">
                ...
                <msh:row>
                    <td class="label" title="Сортировать  (orderBy)" colspan="1"><label for="ordeByName"
                                                                                        id="orderByLabel">Сортировка:</label>
                    </td>
                    <td onclick="this.childNodes[1].checked='checked';">
                ...
                <msh:row>
                    <td class="label" title="Сортировать  (orderByType)" colspan="1"><label for="orderByTypeName"
                                                                                            id="orderByTypeLabel">Отобразить:</label>
                    </td>
                    <td onclick="this.childNodes[1].checked='checked';">
 ...
 ,CAST(EXTRACT (YEAR from (p.birthday)) as int) as y
    	...
    	left join patient p on p.id=dc.patient_id
     	where ${showbyType} ${status} ${statusNoActuality} ${dateGroup } between cast('${beginDate}' as date) and cast('${endDate}' as date) ${disReason} ${disReason2} ${closeReason} ${primarity} ${anotherlpu} order by ${orderBystatus} "/>
                <msh:table printToExcelButton="Сохранить в excel" viewUrl="entityShortView-dis_document.do"
                           name="journal_priem" action="entityParentView-dis_document.do" idField="1">
