        ...
        <msh:form action="entitySaveGoView-mis_patientFondCheckData.do" defaultField="id">
            ...
            <msh:panel>
                <msh:ifFormTypeIsCreate formName="mis_patientFondCheckDataForm">
                    <msh:row>
                        <td class="label" title="Обновлять данных пациентов (typePatient)" colspan="1"><label
                                for="typePatientName" id="typePatientLabel">Обновлять данные:</label></td>
                    ...
                    <msh:row>
                        ...
                        <msh:checkBox property="needUpdateAttachment"
                                      label="Автоматически обновлять данные прикрепления"/>
                    </msh:row>
                ...
                <msh:ifFormTypeAreViewOrEdit formName="mis_patientFondCheckDataForm">
                    ...
                    <msh:row>
                        <msh:checkBox property="needUpdatePolicy" label="Автоматически обновлять данные мед. полиса"
                                      viewOnlyField="true"/>
                        <msh:checkBox property="needUpdateAttachment"
                                      label="Автоматически обновлять данные прикрепления" viewOnlyField="true"/>
                    </msh:row>
...
order by pf.lastname, pf.firstname, pf.middlename
 "/>
                    <msh:table name="dataList" action="javascript:void(0)" idField="1">
                        ...
                        <msh:tableButton property="13" hideIfEmpty="true" buttonFunction="updatePatient"
                                         addParam="1,0,0,0);this.style.display='none';javascript:void(0"
                                         buttonShortName="O" buttonName="Обновить данные пациента"/>
                        <msh:tableColumn columnName="Данные МедОС" property="4"/>
                        <msh:tableColumn columnName="Данные ФОМС" property="3"/>
                        <msh:tableButton property="11" hideIfEmpty="true" buttonFunction="updatePatient"
                                         addParam="0,1,0,0);this.style.display='none';javascript:void(0"
                                         buttonShortName="O" buttonName="Обновить данные документа"/>
                        <msh:tableColumn columnName="Пасп. данные МедОС" property="6"/>
                        <msh:tableColumn columnName="Пасп. данные ФОМС" property="5"/>
                        <msh:tableButton property="12" hideIfEmpty="true" buttonFunction="updatePatient"
                                         addParam="0,0,1,0);this.style.display='none';javascript:void(0"
                                         buttonShortName="O" buttonName="Обновить данные полиса ОМС"/>
                        <msh:tableColumn columnName="Полис ОМС МедОС" property="8"/>
                        <msh:tableColumn columnName="Полис ОМС ФОМС" property="7"/>
                        <msh:tableButton property="14" hideIfEmpty="true" buttonFunction="updatePatient"
                                         addParam="0,0,0,1);this.style.display='none';javascript:void(0"
                                         buttonShortName="O" buttonName="Обновить данные прикрепления"/>
            ...
            <div id='notFoundRecords'>
                <msh:section title="Пациенты, не найденные в базе ФОМС">
                    ...
                    , coalesce(p.patientSync,'') as f6_patientSync
            from journalpatientfondcheck jpfc
            left join patient p on p.id=jpfc.patient_id
            where jpfc.checkTime_id=${param.id} and jpfc.isFound='0' and jpfc.patient_id is not null
            order by jpfc.lastname, jpfc.firstname, jpfc.middlename
 "/>
                    <msh:table name="dataList" action="entityView-mis_patient.do" idField="1">
                        <msh:tableColumn columnName="#" property="sn"/>
                        <msh:tableColumn columnName="Код пациента" property="6"/>
