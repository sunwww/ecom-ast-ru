...
<tiles:insert page="/WEB-INF/tiles/main${param.short}Layout.jsp" flush="true">
    ...
    <tiles:put name="body" type="string">
        ${isReportBase}<ecom:webQuery isReportBase="${isReportBase}" name="sex_woman_sql"
                                      nativeSql="select id,name from VocSex where omccode='2'"/>
        ${isReportBase}<ecom:webQuery isReportBase="${isReportBase}" name="result_death_sql"
                                      nativeSql="select id,name from VocHospitalizationResult where code='11'"/>
        ${isReportBase}<ecom:webQuery isReportBase="${isReportBase}" name="orderType_amb_sql"
                                      nativeSql="select id,name from Omc_Frm where voc_code='К'"/>
        ${isReportBase}<ecom:webQuery isReportBase="${isReportBase}" name="diag_typeReg_direct_sql"
                                      nativeSql="select id,name from VocDiagnosisRegistrationType where code='2'"/>
        ${isReportBase}<ecom:webQuery isReportBase="${isReportBase}" name="diag_typeReg_cl_sql"
                                      nativeSql="select id,name from VocDiagnosisRegistrationType where code='3'"/>
        ${isReportBase}<ecom:webQuery isReportBase="${isReportBase}" name="diag_typeReg_pat_sql"
                                      nativeSql="select id,name from VocDiagnosisRegistrationType where code='5'"/>
        ${isReportBase}<ecom:webQuery isReportBase="${isReportBase}" name="diag_priority_m_sql"
                                      nativeSql="select id,name from VocPriorityDiagnosis where code='1'"/>
        <%
            ...
            paramSql.append(" ").append(ActionUtil.setParameterManyFilterSql("departments", "departments"
                    , "5".equals(view) ? "sls.department_id" : "sloa.department_id", request));
        ...
        <msh:form action="/stac_report_14.do" defaultField="dateBegin" disableFormDataConfirm="true" method="GET">
            ...
            <msh:panel>
                ...
                <msh:row>
                    <td class="label" title="Возраст считать нам момент (typeDate)" colspan="1"><label
                            for="typeDateName" id="typeDateLabel">Возраст считать:</label></td>
                ...
                <msh:row>
                    <td class="label" title="Возрастная группа (typeAge)" colspan="1"><label for="typeAgeName"
                                                                                             id="typeAgeLabel">Возрастная
                        группа:</label></td>
                ...
                <msh:row>
                    <td class="label" title="Просмотр данных (typeView)" colspan="1"><label for="typeViewName"
                                                                                            id="typeViewLabel">Отобразить:</label>
                    </td>
                    <td onclick="this.childNodes[1].checked='checked';" colspan="2">
                ...
                <msh:row>
                    <ecom:oneToManyOneAutocomplete label="Отделения" vocName="vocLpuHospOtdAll" property="departments"
                                                   colSpan="10"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete property="hospType" fieldColSpan="4" horizontalFill="true" label="Тип стационара"
                                      vocName="vocHospType"/>
                </msh:row>
                <msh:row>
                ...
                <msh:row>
                    <msh:autoComplete property="sex" fieldColSpan="4" horizontalFill="true" label="Пол"
                                      vocName="vocSex"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete property="additionStatus" fieldColSpan="4" horizontalFill="true"
                                      label="Доп.статус пациента" vocName="vocAdditionStatus"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete property="bedType" fieldColSpan="2" horizontalFill="true" label="Профиль коек"
                                      vocName="vocBedType"/>
                    <msh:autoComplete property="bedSubType" fieldColSpan="2" horizontalFill="true" label="Тип коек"
                                      vocName="vocBedSubType"/>
                </msh:row>
                <msh:row>
    ...
    "/>
                <msh:table printToExcelButton="Сохранить в excel" name="report14swod"
                           ...
                           action="stac_report_14.do?${paramHref}&additionStatus=${param.additionStatus}&typeAge=${typeAge}&typeView=${typeView}&typeAge=${typeAge}&noViewForm=1&period=${dateBegin}-${dateEnd}"
                           idField="1">
                    ...
                    <msh:tableColumn isCalcAmount="true" columnName="из них доставленых по экстренным показаниям"
                                     property="4"/>
                    <msh:tableColumn isCalcAmount="true"
                                     columnName="из них экст. пациентов, доставленных скорой мед.помощью" property="5"/>
    ...
    ,case when sls.emergency='1' and sls.orderType_id='${orderType_amb}' then 'Да' else null end as f12_emerSk
    ,vho.name as f13_vhoname
...
"/>
                <msh:table printToExcelButton="Сохранить в excel" name="report14swod"
                           ...
                           action="stac_report_14.do?${paramHref}${departmentsUrlId}&typeAge=${typeAge}&typeView=${typeView}&typeAge=${typeAge}&noViewForm=1&period=${dateBegin}-${dateEnd}"
                           idField="1">
                    ...
                    <msh:tableColumn isCalcAmount="true" columnName="из них доставленых по экстренным показаниям"
                                     property="10"/>
                    <msh:tableColumn isCalcAmount="true"
                                     columnName="из них экст. пациентов, доставленных скорой мед.помощью"
                                     property="11"/>
...
order by p.lastname,p.firstname,p.middlename "/>
        ...
        <msh:section>
            ${isReportBase}<ecom:webQuery isReportBase="${isReportBase}" name="report14swod"
                                          nameFldSql="report14swod_sql" nativeSql="
            ...
            <msh:sectionTitle>Свод по нозоологиям (${outcomeInfo})
                ...
                <form action="print-report_14_2.do" method="post" target="_blank">
                    ...
                    <input type='hidden' name="sqlInfo" id="sqlInfo"
                           value="Свод по нозоологиям (${outcomeInfo}) за ${param.dateBegin}-${dateEnd}.">
                    <input type='hidden' name="sqlColumn" id="sqlColumn" value="${groupName}">
                    <input type='hidden' name="s" id="s" value="PrintService"><input type='hidden' name="isReportBase"
                                                                                     id="isReportBase"
                                                                                     value="${isReportBase}">
...
order by p.lastname,p.firstname,p.middlename "/>
                <form action="print-stac_report14_r2.do" method="post" target="_blank">
                    ...
                    <input type='hidden' name="sqlInfo" id="sqlInfo"
                           value="Реестр пациентов (${outcomeInfo}) ${param.strname} за ${param.dateBegin}-${dateEnd}.">
                    <input type='hidden' name="sqlColumn" id="sqlColumn" value="${groupName}">
                    <input type='hidden' name="s" id="s" value="PrintService"><input type='hidden' name="isReportBase"
                                                                                     id="isReportBase"
                                                                                     value="${isReportBase}">
...
case when dc.categoryDifference_id is not null or dc.latrogeny_id is not null then sls.id else null end    ) as c6ntRash
    ...
    "/>
                <form action="print-stac_report_14_3.do" method="post" target="_blank">
                    ...
                    <input type='hidden' name="sqlText" id="sqlText" value="${report14swod_sql}">
                    <input type='hidden' name="sqlInfo" id="sqlInfo"
                           value="Свод по нозоологиям (умершие) за ${param.dateBegin}-${dateEnd}.">
                    <input type='hidden' name="sqlColumn" id="sqlColumn" value="${groupName}">
                    <input type='hidden' name="s" id="s" value="PrintService"><input type='hidden' name="isReportBase"
                                                                                     id="isReportBase"
                                                                                     value="${isReportBase}">
        ...
        <msh:section>
            <msh:sectionTitle>
                ${isReportBase}<ecom:webQuery isReportBase="${isReportBase}" name="journal_surOperation"
                                              nameFldSql="journal_reestr_sql" nativeSql="
    select 
    sls.id as slsid
    ,ss.code as sscode
    ,p.lastname||' '||p.firstname||' '||p.middlename as fio
...
,'№ '||cert.number ||'-'|| cert.series as certofdeath
    ...
    order by p.lastname,p.firstname,p.middlename "/>
                <form action="print-stac_report_14_3r.do" method="post" target="_blank">
                    ...
                    <input type='hidden' name="sqlInfo" id="sqlInfo"
                           value="Реестр пациентов ${param.strname} по нозоологиям (умершие) за ${param.dateBegin}-${dateEnd}.">
                    <input type='hidden' name="sqlColumn" id="sqlColumn" value="${groupName}">
                    <input type='hidden' name="s" id="s" value="PrintService"><input type='hidden' name="isReportBase"
                                                                                     id="isReportBase"
                                                                                     value="${isReportBase}">
...
case when dc.categoryDifference_id is not null or dc.latrogeny_id is not null then sls.id else null end    ) as c6ntRash
    ...
    "/>
                <form action="print-stac_report_14_3.do" method="post" target="_blank">
                    ...
                    <input type='hidden' name="sqlText" id="sqlText" value="${report14swod_sql}">
                    <input type='hidden' name="sqlInfo" id="sqlInfo"
                           value="Свод по нозоологиям (умершие) ДОСУТОЧНО за ${param.dateBegin}-${dateEnd}.">
                    <input type='hidden' name="sqlColumn" id="sqlColumn" value="${groupName}">
                    <input type='hidden' name="s" id="s" value="PrintService"><input type='hidden' name="isReportBase"
                                                                                     id="isReportBase"
                                                                                     value="${isReportBase}">
...
order by p.lastname,p.firstname,p.middlename "/>
        ...
        <msh:section>
            <msh:sectionTitle>
                <ecom:webQuery isReportBase="${isReportBase}" name="report14swod" nameFldSql="report14swod_sql"
                               nativeSql="
...
"/>
                ...
                <form action="print-report_14_7.do" method="post" target="_blank">
                    ...
                    <input type='hidden' name="sqlText" id="sqlText" value="${report14swod_sql}">
                    <input type='hidden' name="sqlInfo" id="sqlInfo"
                           value="Свод по операциям за ${dateBegin}-${dateEnd}.">
                    <input type='hidden' name="sqlColumn" id="sqlColumn" value="${groupName}">
                    <input type='hidden' name="s" id="s" value="PrintService"><input type='hidden' name="isReportBase"
                                                                                     id="isReportBase"
                                                                                     value="${isReportBase}">
