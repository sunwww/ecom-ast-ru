...
<tiles:insert page="/WEB-INF/tiles/main${param.short}Layout.jsp" flush="true" >
    <tiles:put name='title' type='string'>
        <msh:title mainMenu="StacJournal">Выполнение плана по КСГ по профилю</msh:title>
    </tiles:put>
    <tiles:put name='side' type='string'>
    </tiles:put>
    <tiles:put name="body" type="string">
        <msh:form action="/finPlanByProfile.do" defaultField="dateBegin" disableFormDataConfirm="true" method="GET">
            <msh:panel>
                <msh:row>
                ...
                if ($('dateBegin').value!=null && $('dateBegin').value!="" && $('dateEnd').value!=null && $('dateEnd').value!="") {
                    HospitalMedCaseService.getSettingsKeyValueByKey("jasperServerUrl", {
                        callback: function (res) {
                            ...
                            window.location.href = "finPlanByProfile.do?dateBegin="+$('dateBegin').value+"&dateEnd="+$('dateEnd').value+"&hospType="+$('hospType').value+"&filterAdd="+$('filterAdd').value;
                            if (res != "##") {
                                var bdt=($('hospType').value!="")? "&bedtype=" + $('hospType').value:"";
                                var profilek=($('filterAdd').value!="")? "&prname=" + $('filterAdd').value:"";
                                window.open("http://" + resMas[0] + "/jasperserver/flow.html?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2Freports&reportUnit=%2Freports%2FfinPlan1&standAlone=true&decorate=no"
