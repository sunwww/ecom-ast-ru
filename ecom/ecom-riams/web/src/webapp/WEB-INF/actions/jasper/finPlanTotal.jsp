...
<tiles:insert page="/WEB-INF/tiles/main${param.short}Layout.jsp" flush="true" >
    ...
    <tiles:put name="body" type="string">
        <msh:form action="/finPlanTotal.do" defaultField="dateBegin" disableFormDataConfirm="true" method="GET">
            <msh:panel>
                <msh:row>
                ...
                if ($('dateBegin').value!=null && $('dateBegin').value!="" && $('dateEnd').value!=null && $('dateEnd').value!="") {
                    HospitalMedCaseService.getSettingsKeyValueByKey("jasperServerUrl", {
                        callback: function (res) {
                            ...
                            window.location.href = "finPlanTotal.do?dateBegin="+$('dateBegin').value+"&dateEnd="+$('dateEnd').value+"&hospType="+$('hospType').value+"&department="+$('department').value;
                            if (res != "##") {
                                var bdt=($('hospType').value!="")? "&bedtype=" + $('hospType').value:"";
                                var lpu=($('department').value!="")? "&lpu=" + $('department').value:"";
                                window.open("http://" + resMas[0] + "/jasperserver/flow.html?_flowId=viewReportFlow&_flowId=viewReportFlow&ParentFolderUri=%2Freports&reportUnit=%2Freports%2FfinPlan3&standAlone=true&decorate=no"
