...
<tiles:insert page="/WEB-INF/tiles/mainLayout.jsp" flush="true" >
  ...
  <tiles:put name="title" type="string">
    <msh:title mainMenu="PatientJournal" title="Получение документов" />
  </tiles:put>
  ...
  <tiles:put name="body" type="string">
    ...
    String typeOrderBy =ActionUtil.updateParameter("FLOW_RECEIT_Journal","typeOrderBy","1", request) ;
  	...
  	%>
  	  ...
  	  <msh:form action="/doc_flow_receit.do" defaultField="beginDate" disableFormDataConfirm="true" method="GET">
...
,vr.name as f4vrname,coalesce('ИБ№'||ss.code,'МК№'||mc.number,'') as f5infodoc
 ...
 ,to_char(fd.posteddate,'dd.mm.yyyy') as f9createdate 
 ,fd.createusername as f10createusername,su.fullname as f11fullname
 ,to_char(fd.posteddate,'dd.mm.yyyy') as f12posteddate
 , case when fd.postedDate is not null and fd.receiptDate is not null then fd.id else null end as f13fdid
...
left join VocFlowDocmentPlace vfdp2 on vfdp2.id=fd.placeTo_id
    ...
    <form  id="printForm" name="printForm" action="print-doc_flow_sending.do" method="post" target="_blank">
	 Список пациентов за ${beginDate}-${finishDate}   
	    ...
	    <input type='hidden' name="sqlInfo" id="sqlInfo" value='Список доставленных документов за ${beginDate}-${finishDate}'>
    ...
    <msh:sectionContent>
    <tags:pres_intakeDate name="Biomat" service="PatientService" method="updateReceitDateByFlowDoc"/>
            ...
            <tr>
              <th colspan="15">
                <msh:toolbar>
                  ...
                  <a href="javascript:transferIn()">Зарегистрировать получение документов</a>
                </msh:toolbar>
              </th>
            </tr>
            </msh:ifInRole>
          </msh:tableNotEmpty>
          <msh:tableButton hideIfEmpty="true" property="13" buttonFunction="deleteDataByFlowDoc" buttonName="Удалить получение" buttonShortName="Удалить получение" role="/Policy/Mis/Document/Flow/DeleteReceit"/>
  ...
  <tiles:put name="javascript" type="string">
  	...
  	function deleteDataByFlowDoc(aId) {
  		if (confirm("Вы действительно хотите удалить получение документа?")) {
  		PatientService.deleteDataByFlowDoc(aId,2, { 
