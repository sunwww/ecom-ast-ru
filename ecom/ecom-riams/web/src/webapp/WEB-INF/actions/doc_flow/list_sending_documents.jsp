...
<tiles:insert page="/WEB-INF/tiles/mainLayout.jsp" flush="true" >
  ...
  <tiles:put name="title" type="string">
    <msh:title mainMenu="PatientJournal" title="Отправка документов" />
  </tiles:put>
  ...
  <tiles:put name="body" type="string">
  	...
  	String typeMoveFrom =ActionUtil.updateParameter("FLOW_SENDING_Journal","typeMoveFrom","2", request) ;
  	String typeMoveTo =ActionUtil.updateParameter("FLOW_SENDING_Journal","typeMoveTo","1", request) ;
    String typeDocument =ActionUtil.updateParameter("FLOW_SENDING_Journal","typeDocument","4", request) ;
    String typeIsSending =ActionUtil.updateParameter("FLOW_SENDING_Journal","typeIsSending","2", request) ;
    String typeIsReceiting =ActionUtil.updateParameter("FLOW_SENDING_Journal","typeIsReceiting","2", request) ;
    String typeOrderBy =ActionUtil.updateParameter("FLOW_SENDING_Journal","typeOrderBy","1", request) ;
  	...
  	%>
  	  ...
  	  <msh:form action="/doc_flow_sending.do" defaultField="beginDate" disableFormDataConfirm="true" method="GET">
    <msh:panel guid="6ae283c8-7035-450a-8eb4-6f0f7da8a8ff">
      ...
      <msh:row>
        <td class="label" title="Статус отправки (typeIsSending)" colspan="1"><label for="typeIsSendingName" id="typeIsSendingLabel">Статус отправки:</label></td>
        <td onclick="this.childNodes[1].checked='checked';checkfrm();this.form.submit() ;">
        	<input type="radio" name="typeIsSending" value="1"> отправлено
        </td>
        <td onclick="this.childNodes[1].checked='checked';checkfrm();this.form.submit() ;" >
        	<input type="radio" name="typeIsSending" value="2"> не отправлено
        </td>
        <td onclick="this.childNodes[1].checked='checked';checkfrm();" colspan="2">
        	<input type="radio" name="typeIsSending" value="3"> отобразить все данные
        </td>
     </msh:row>
...
,vr.name as f4vrname,coalesce('ИБ№'||ss.code,'МК№'||mc.number,'') as f5infodoc
 ...
 ,to_char(fd.createdate,'dd.mm.yyyy') as f9createdate ,fd.createusername as f10createusername,su.fullname as f11fullname
 , case when fd.postedDate is null then fd.id else null end as f12fdid
 , case when fd.postedDate is not null and fd.receiptDate is null then fd.id else null end as f13fdid
 , fd.patient as f14patient
...
left join VocFlowDocmentPlace vfdp2 on vfdp2.id=fd.placeTo_id
    where fd.createdate between to_date('${beginDate}','dd.mm.yyyy') 
    ...
    <form  id="printForm" name="printForm" action="print-doc_flow_sending.do" method="post" target="_blank">
	 Список пациентов за ${beginDate}-${finishDate}   
	    <input type='hidden' name="sqlText" id="sqlText" value="${list_sql}"> 
	    <input type='hidden' name="sqlInfo" id="sqlInfo" value='Список документов на отправку за ${beginDate}-${finishDate}'>
	    ...
	    function printSomePrescriptions() {
	    	...
	    	document.getElementById('printForm').action='print-doc_flow_sending.do';
    ...
    <msh:sectionContent>
    <tags:pres_intakeDate name="Biomat" service="PatientService" method="updatePostedDateByFlowDoc"/>
	    <msh:table name="list" idField="1" action="javascript:void(0)" selection="multiply">
	    <msh:tableNotEmpty>
	     <msh:ifInRole roles="/Policy/Mis/Document/Flow/Edit">
            <tr>
              <th colspan="15">
                <msh:toolbar>

                  <a href="javascript:transferIn()">Зарегистрировать передачу документов</a>
                </msh:toolbar>
              </th>
            </tr>
            </msh:ifInRole>
          </msh:tableNotEmpty>
          <msh:tableButton hideIfEmpty="true" property="12" buttonFunction="deleteDataByFlowDoc" buttonName="Удалить" buttonShortName="Удалить" role="/Policy/Mis/Document/Flow/Delete"/>
          <msh:tableButton hideIfEmpty="true" property="13" buttonFunction="deleteDataPostedByFlowDoc" buttonName="Удалить отправку" buttonShortName="Удалить отправку" role="/Policy/Mis/Document/Flow/DeletePosted"/>
          <msh:tableButton hideIfEmpty="true" property="14" buttonFunction="viewPatient" buttonName="Пациент" buttonShortName="Пациент" role="/Policy/Mis/Patient/View"/>
  ...
  <tiles:put name="javascript" type="string">
  	...
  	function transferIn() {
  		...
  		function deleteDataByFlowDoc(aId) {
  		...
  		function deleteDataPostedByFlowDoc(aId) {
  			if (confirm("Вы действительно хотите удалить отправку документа?")) {
  			PatientService.deleteDataByFlowDoc(aId,1, { 
		            callback: function(aResult) {
		            	window.document.location.reload();
		            }
				});
