...
<tiles:insert page="/WEB-INF/tiles/mainLayout.jsp" flush="true">
            ...
            <msh:panel>
                ...
                group by e.entryType, e.billDate, e.billNumber ,e.serviceStream, e.isForeign,e.bill_id,bill.comment, e.fileType, le.startDate , le.finishDate, bill.billNumber,bill.sum, e.addGroupFld
                 order by e.entryType, e.serviceStream, e.billDate, e.billNumber  "/>
                    ...
                    <msh:table idField="1" name="entries" action="entityParentList-e2_entry.do"
                               noDataMessage="Нет записей по заполнению">
                        ...
                        <msh:tableButton property="11" hideIfEmpty="true" buttonShortName="©"
                                         buttonFunction="printBill"/>
                        <msh:tableButton property="11" hideIfEmpty="true" buttonShortName="©_"
                                         buttonFunction="printReestr"/>
                        ...
                        <msh:tableButton property="1" buttonShortName="Присвоить счет" buttonFunction="showE2BillDialog"
                                         addParam="this" role="/Policy/E2/Admin"/>
                        <msh:tableButton property="1" buttonShortName="Сформировать пакет" buttonFunction="createMPFile"
                                         addParam="this" role="/Policy/E2/Admin"/>
                        <msh:tableButton property="1" buttonShortName="Проверить случаи по записи"
                                         buttonFunction="makeCheck" addParam="this" role="/Policy/E2/Admin"/>
                    </msh:table>
                </msh:ifFormTypeIsView>
    ...
    <tiles:put name="side" type="string">
        <msh:ifFormTypeIsView formName="e2_entryListForm">
            <msh:sideMenu>
                <msh:sideLink key="ALT+2" params="id" action="/entityEdit-e2_entryList" name="Изменить"
                ...
                <msh:sideLink action="/javascript:showExportPaketHistory" name="Просмотреть выгруженные пакеты"
                              roles="/Policy/E2/View"/>
                <msh:ifPropertyIsTrue formName="e2_entryListForm" propertyName="isClosed" invert="true">
                    <msh:sideLink action="/javascript:makeCheck(null,this)" name="Проверить все случаи"
                                  roles="/Policy/E2/View"/>
                    <msh:sideLink key="ALT+DEL" confirm="Удалить?" params="id" action="/entityDelete-e2_entryList"
                                  name="Удалить" roles="/Policy/E2/Delete"/>
                    <msh:sideLink action="/javascript:showImportFileBillDialog()" name="Импортировать ответ фонда"
                                  roles="/Policy/E2/Edit"/>
                    <msh:sideLink action="/javascript:exportDefectNewListEntry()"
                                  name="Выгрузить дефекты в новое заполнение" roles="/Policy/E2/Edit"/>
                    <msh:sideLink action="/javascript:markAsReSend(false)" name="Пометить как первую подачу"
                                  roles="/Policy/E2/Edit"/>
                    <msh:sideLink action="/javascript:markAsReSend(true)" name="Пометить как повторную подачу"
                                  roles="/Policy/E2/Edit"/>
                    <msh:sideLink action="/javascript:closeListEntry(true)" name="Закрыть заполнение"
                                  roles="/Policy/E2/Admin"/>
                    <msh:sideLink action="/javascript:addHistoryNumberToList()"
                                  name="Добавить в заполнение госпитализацию" roles="/Policy/E2/Edit"/>
                    <msh:sideLink action="/javascript:showUnionUnionDialog()" name="Объединить с другим заполнением"
                                  roles="/Policy/E2/Admin"/>
                    <msh:sideLink action="/javascript:showUnionExportHistory()" name="Журнал пакетов по счетам"
                                  roles="/Policy/E2/Edit"/>
                    <msh:sideLink action="/javascript:refillListEntry()" name="Переформировать заполнение"
                                  roles="/Policy/E2/Admin"/>
                    <msh:sideLink action="/javascript:setDirectAndPlanHospDate()"
                                  name="Заполнить пустые даты направления и даты пред. госпитализации"
                                  roles="/Policy/E2/Admin"/>
                    <msh:sideLink action="/javascript:showSplitForeignOtherBill()"
                                  name="Выделить иногородних отдельным счетом" roles="/Policy/E2/Admin"/>
                    <msh:sideLink action="/javascript:exportToCentralSegment()" name="Сделать запрос в ЦС"
                                  roles="/Policy/E2/Edit"/>
                    <msh:sideLink action="/javascript:showCancerCancerDialog()" name="Сделать онкослучай"
                                  roles="/Policy/E2/Admin"/>
                </msh:ifPropertyIsTrue>
                <msh:sideLink action="/javascript:closeListEntry(false)" name="Открыть заполнение"
                              roles="/Policy/E2/Admin"/>
        ...
        <msh:ifFormTypeIsView formName="e2_entryListForm">
            ...
            <script type="text/javascript">
                ...
                function showSplitForeignOtherBill() {
                    jQuery('#E2Save').click(function () {
                        splitForeignOtherBill();
                    });
                    showE2BillDialog('');
                ...
                function closeListEntry(aClose) {
                    Expert2Service.closeListEntry(${param.id}, aClose, {
                        callback: function () {
                            alert(aClose ? "Закрыто!" : "Открыто");
                            window.document.location.reload()
                        }
                    });
                }
