    ...
    <tiles:put name="body" type="string">
        ...
        <msh:form action="/pres_journal_intake.do" defaultField="beginDate" disableFormDataConfirm="true" method="GET">
            <msh:panel>
                ...
                <msh:row>
                    <td class="label" title="Передача в лабораторию (typeTransfer)"><label for="typeTransferName"
                                                                                           id="typeTransferLabel">Передача
                        в лабораторию:</label></td>
        ...
        <%
            if (typeGroup.equals("2")) {
                request.setAttribute("addByGroup", "p.id,");
            }
   ...
   ,vst.name as vstname
    ...
    "/>
            ...
            <msh:sectionTitle>
                ...
                <form id="printForm" name="printForm" action="print-pres_lab_prescript_by_department.do" method="post"
                      target="_blank">
    ...
    order by vsst.name,pat.lastname,pat.firstname,pat.middlename">
                    <input type='hidden' name="sqlInfo" id="sqlInfo"
                           value='Список пациентов за ${beginDate}-${endDate} по отделению ${lpu_name}'>
                    ...
                    <input type="button" value="Печать" title="Печать всего списка (либо тех, что отмечены галочками)"
                           onclick="printSomePrescriptions()">
                    <input type="submit" value="Печать этикеток"
                           onclick="this.form.action='print-pres_lab_prescript_by_department_birok.do'">
                    <script type="text/javascript">
                        function printSomePrescriptions() {
                            ...
                            $('sqlText').value = "select pat.id as f1pat 	,case when p.intakeDate is null then '' else '' end as f2js " +
                                ...
                                " ,to_char(pat.birthday,'dd.mm.yyyy') as f9birthday " +
                                " ,list(case when vst.code='LABSURVEY' then ms.code||coalesce('('||ms.additionCode||')','')||' '||ms.name" +
                                "||(case when p.noteforlab is not null and p.noteforlab<>'' then ' ('||coalesce(p.noteforlab,'')||')' else '' end) else null end) as f10medServicies " +
                                " ,list(distinct wp.lastname||' '||wp.firstname||' '||wp.middlename) as f11fioworker " +
                                " ,list(distinct iwp.lastname||' '||iwp.firstname||' '||iwp.middlename) as f12intakefioworker " +
                                " ,to_char(p.intakeDate,'dd.mm.yyyy')||' '||cast(p.intakeTime as varchar(5)) as f13dtintake " +
                                " ,list(ms.additionCode || (select to_char(pmc.datestart,'dd.mm.yyyy')  " +
