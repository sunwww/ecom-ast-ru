    ...
    <tiles:put name='body' type='string'>
        ...
        <msh:hideException>
            <msh:section title='Результат поиска'>
                <ecom:webQuery name="listAll" nativeSql="select voc.id, voc.code, voc.name, voc.parent, list(vp.code||' '||vp.name) as f5_vocPost
                , list(v021.code||' '||v021.name) as f6_v021
                from voce2fondv015 voc
  left join e2fondmedspeclink link on link.medspec_id=voc.id
  left join vocpost vp on vp.code=link.medosworkfunction
  left join voce2fondv021 v021 on v021.id=voc.medSpecV021_id
  group by voc.id, voc.code, voc.name"/>
                <msh:table  name="listAll" action="entityView-e2_vocFondV015.do" idField="1" disableKeySupport="true">
                    ...
                    <msh:tableColumn columnName="Специальность V021"  property="6" guid="5b05897f-5dfd-4aee-ada9-d04244ef20c6" />
                </msh:table>
            </msh:section>
        </msh:hideException>
    </tiles:put>
