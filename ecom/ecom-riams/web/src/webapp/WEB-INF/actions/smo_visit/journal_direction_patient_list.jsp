...
<tiles:insert page="/WEB-INF/tiles/mainLayout.jsp" flush="true">
    ...
    <tiles:put name="body" type="string">
        <msh:form action="/visit_journal_direction.do" defaultField="beginDate" disableFormDataConfirm="true"
                  method="GET">
            ...
            <msh:panel colsWidth="1%,1%,1%">
                ...
                <msh:row>
                    <msh:autoComplete property="orderWorkFunction" vocName="workFunction"
                                      horizontalFill="true" fieldColSpan="10" size="70"/>
                </msh:row>
                <tr>
                    <td colspan="12">
                        <table>
                            <msh:row>
                                <td class="label" title="Искать по дате (typeDate)" colspan="1"><label
                                        for="typeDateName" id="typeDateLabel">Искать по дате:</label></td>
                            ...
                            <msh:row>
                                <td class="label" title="Искать по пользователям (typeUser)" colspan="1"><label
                                        for="typeUserName" id="typeUserLabel">Пользователи:</label></td>
                            ...
                            <msh:row>
                                <td class="label" title="Группировака (typePatient)" colspan="1"><label
                                        for="typeGroupName" id="typeGroupLabel">Группировка по:</label></td>
                                <td onclick="this.childNodes[1].checked='checked';" colspan="2">
                            ...
                            <msh:row>
                                <td class="label" title="Просмотр данных (typeView)" colspan="1"><label
                                        for="typeViewName" id="typeViewLabel">Отобразить:</label></td>
                                <td onclick="this.childNodes[1].checked='checked';" colspan="2">
                                    <input type="radio" name="typeView" value="1"> реестр направл.
                                </td>
                                <td onclick="this.childNodes[1].checked='checked';" colspan="2">
                                    <input type="radio" name="typeView" value="2"> свод направл.
                                </td>
                                <td onclick="this.childNodes[1].checked='checked';" colspan="3">
                                    <input type="radio" name="typeView" value="3"> свод направл. по районам
                                </td>
                                <td onclick="this.childNodes[1].checked='checked';" colspan="4">
                                    <input type="radio" name="typeView" value="4"> свод лаб. услуг
                                </td>
                            </msh:row>
                        </table>
                    </td>
                ...
                <msh:row>
                    <td colspan="3" class="buttons">
                        <input type="button" title="Найти"
                               onclick="this.value=&quot;Поиск...&quot;;  this.form.action=&quot;visit_journal_direction.do&quot;;this.form.target=&quot;&quot; ; this.form.submit(); return true ;"
                               value="Найти" class="default" id="submitButton" autocomplete="off">
                    </td>
        ...
        <msh:section>
            ...
            <msh:sectionContent>
                <ecom:webQuery isReportBase="true" name="journal_reestr" nameFldSql="qqq" nativeSql="
select t.id as tid
, lpu.name as lpuname
...
"/>
                <msh:table printToExcelButton="в excel"
                           name="journal_reestr" viewUrl="entityShortView-smo_visit.do" action="entityView-smo_visit.do"
        ...
        <msh:section>
            ...
            <msh:sectionContent>
                <ecom:webQuery isReportBase="true" name="journal_swod" nativeSql="
${queryTextBegin}
...
"/>
                <msh:table printToExcelButton="excel"
                           name="journal_swod" action="visit_journal_direction.do" idField="1"
...
"/>
                <msh:table printToExcelButton="excel"
                           name="journal_swod_rayon" action="visit_journal_direction.do" idField="1"
                           noDataMessage="Не найдено">
                    <msh:tableColumn columnName="#" property="sn"/>
                    <msh:tableColumn columnName="${groupByTitle}" property="2"/>
                    <msh:tableColumn columnName="Кол-во напр. пац." property="3"/>
        ...
        <msh:section>
            <msh:sectionTitle>Период с ${beginDate} по ${finishDate}. ${filterInfo} ${specInfo} ${workFunctionInfo} ${lpuInfo} ${serviceStreamInfo}</msh:sectionTitle>
            <msh:sectionContent>
                <ecom:webQuery isReportBase="true" name="journal_swod_lab" nativeSql="
select t.id as tid
,vss.name as vssname
,ovwf.name||' '||owp.lastname||' '||owp.firstname||' '||owp.middlename as owfinfo
, p.lastname||' '||p.firstname||' '||coalesce(p.middlename) as fiopat
