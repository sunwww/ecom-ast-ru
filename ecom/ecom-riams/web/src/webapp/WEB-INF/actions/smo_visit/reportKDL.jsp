    ...
    <tiles:put name="body" type="string">
        <%
            ...
            if (typeVMPOrNot != null) {
                ...
                request.setAttribute("typeVMPOrNotValue", "");
            }
        ...
        <msh:form action="/reportKDL.do" defaultField="departments" disableFormDataConfirm="true" method="GET">
            <msh:panel>
                ...
                <msh:row>
                    <td class="label" title="Поиск по промежутку  (typeStacOrNot)" colspan="1"><label
                            for="typeStacOrNotName" id="ttypeStacOrNotLabel">Группировать по:</label></td>
                    ...
                    <td onclick="this.childNodes[1].checked='checked';" colspan="3">
                        <input type="radio" name="typeStacOrNot" value="3"> группировка по отделениям, вывод по типу
                        исследования
                    </td>
                    <td onclick="this.childNodes[1].checked='checked';radio();" colspan="4">
                        <input type="radio" name="typeStacOrNot" value="4" id="radio4"> группировка по пациентам, вывод
                        по ВМП за период
                    </td>
                </msh:row>
                <msh:row>
                    <td class="label" title="Поиск по промежутку  (typeVMPOrNot)" colspan="1"><label
                            for="typeVMPOrNotName" id="typeVMPOrNotLabel">Отобразить ВМП:</label></td>
        ...
        <%
            ...
            String typeStacOrNot2 = request.getParameter("typeStacOrNot");
            if (request.getParameter("dateBegin") != null && !request.getParameter("dateBegin").equals("")) {
                if ("1".equals(typeStacOrNot2) || request.getParameter("short") != null) {
                    if (request.getParameter("short") != null) {
...
"/>
            ...
            <msh:sectionContent>
                <msh:table printToExcelButton="Сохранить в excel" name="total"
                    ...
                    <msh:tableColumn columnName="Количество всего" property="6" isCalcAmount="true"
                                     addParam="&nul=nul"/>
        ...
        <msh:section>
            <msh:sectionTitle>
               ...
               select name,sum(cnt1) as cnt1,sum(cnt2) as cnt2,sum(cnt3) as cnt3,sum(cnt4) as cnt4,sum(cnt5) as cnt5
                ,sum(cnt6) as cnt6,sum(cnt7) as cnt7, sum(cnt8) as cnt8_trance from
                (select dep.name
                ...
                ,case when msPr.code='TRANCE' ${typeVMPOrNotValueNotNull} then count(mc.id) else '0' end as cnt8
                 ,case when msPr.code in ('Q01', 'Q02', 'Q03', 'Q04', 'Q05', 'Q06', 'TRANCE')
                ...
                ${typeVMPOrNotValueLeftJoin}
                where ${dateT} between to_date('${dateBegin}','dd.mm.yyyy') and to_date('${dateEnd}','dd.mm.yyyy')
                ${deps} and pr.canceldate is null and vst.code='LABSURVEY'
...
"/>
            ...
            <msh:sectionContent>
                <msh:table printToExcelButton="Сохранить в excel" name="totalName"
                    ...
                    <msh:tableColumn columnName="Гематологические" property="2" isCalcAmount="true"
                                     addParam="&nul=nul"/>
                    ...
                    <msh:tableColumn columnName="Гемостазиологические" property="5" isCalcAmount="true"
                                     addParam="&nul=nul"/>
                    <msh:tableColumn columnName="Иммунологические" property="6" isCalcAmount="true"
                                     addParam="&nul=nul"/>
                    <msh:tableColumn columnName="Микробиологические" property="7" isCalcAmount="true"
                                     addParam="&nul=nul"/>
                    <msh:tableColumn columnName="Инфекционная иммунология" property="9" isCalcAmount="true"
                                     addParam="&nul=nul"/>
        ...
        } else if ("2".equals(typeStacOrNot2)) {
        %>
        <msh:section>
            <msh:sectionTitle>
                <ecom:webQuery isReportBase="true" name="total" nameFldSql="total_sql" nativeSql="
...
"/>
            ...
            <msh:sectionContent>
                <msh:table printToExcelButton="Сохранить в excel" name="total"
                    ...
                    <msh:tableColumn columnName="#" property="sn" addParam="&nul=nul"/>
                    <msh:tableColumn columnName="Структурные подразделения" property="1"
                                     addParam="&short=Short&dateBegin=${param.dateBegin}&dateEnd=${param.dateEnd}&typeStacOrNot=1"/>
                    <msh:tableColumn columnName="Поступило" property="2" isCalcAmount="true" addParam="&nul=nul"/>
                    <msh:tableColumn columnName="В т.ч. экстренно" property="3" isCalcAmount="true"
                                     addParam="&nul=nul"/>
                    <msh:tableColumn columnName="Кол-во выполненных исслед." property="4" isCalcAmount="true"
                                     addParam="&nul=nul"/>
        ...
        <%
        } else if ("3".equals(typeStacOrNot2)) {
        %>
        <msh:section>
            <msh:sectionTitle>
                <ecom:webQuery isReportBase="true" name="total" nameFldSql="total_sql" nativeSql="
                ...
                ${deps}
                and pr.canceldate is null and vst.code='LABSURVEY'
                group by dep.id, mc.emergency,msPr.code,vst.code ${typeVMPOrNotValueGroup},pr.id
                order by dep.name
                ) as t
                group by name
"/>
        ...
        <%
        } else if ("4".equals(typeStacOrNot2)) {
        %>
        <msh:section>
            <msh:sectionTitle>
                <ecom:webQuery isReportBase="true" name="total" nameFldSql="total_sql" nativeSql="
