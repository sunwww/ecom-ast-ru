    ...
    <tiles:put name='body' type='string'>
        <%
                ...
                for (String field : fields) {
                    ...
                    if (fldValue != null && !fldValue.trim().equals("")) {
                        ...
                        if (fldName.equals("lastname")) {
                            ...
                            if (fio.length > 1) {
                                filterSql.append(" and e.firstname like upper('").append(fio[1]).append("%')");
                            }
                            if (fio.length > 2) {
                                filterSql.append(" and e.middlename like upper('").append(fio[2]).append("%')");
                            }
                            if (fio.length > 3) {
                                filterSql.append(" and e.birthdate =to_date('").append(fio[3]).append("','dd.MM.yyyy')");
                            }
            ...
            if (billDate != null && !billDate.equals("")) {
                sqlAdd.append(" and e.billDate=to_date('").append(billDate).append("','dd.MM.yyyy')");
            }
            ...
            if (errorCode != null && !errorCode.equals("")) {
                ...
                if (showServices) {
                    request.setAttribute("joinServiceSql", " left join entrymedservice ems on ems.entry_id=e.id" +
            ...
            if (isForeign != null) {
                searchWhereSql += " and e.isForeign='" + (isForeign.equals("1") ? "1" : "0") + "'";
            }
        ...
        <msh:panel>
            <table>
                ...
                <tr>
                    ...
                    <td>
                        <input type="button" value="Сортировать по ФИО"
                               onclick="setOrderBy('e.lastname,e.firstname, e.middlename')">
                    </td>
                    <td>
                ...
                <tr>
                    <td colspan="3">
                        <input type="checkbox" id="dontShowComplexCase" name="dontShowComplexCase"
                               onclick="dontShow(this)">

                        <input type="button" onclick="exportErrorsNewListEntry()"
                               value="Перенести ошибки в новое заполнение">
  ...
  order by ${orderBySql} "/>
        <msh:hideException>
            <msh:section title='Результат поиска ${searchTitle}'>
                <msh:table name="entries" printToExcelButton="в excel" action="entityParentView-e2_entry.do" idField="1"
                           disableKeySupport="true" styleRow="12" cellFunction="true" openNewWindow="true">
