...
<tiles:insert page="/WEB-INF/tiles/mainLayout.jsp" flush="true">
    ...
    <tiles:put name="body" type="string">
        ...
        <msh:form action="/entityParentSaveGoView-e2_entry.do" defaultField="lastname">
            ...
            <msh:panel>
                ...
                <msh:row>
                    <msh:autoComplete property="medPolicyType" vocName="VocMedPolicyTypeCode" horizontalFill="true"
                                      fieldColSpan="6"/>
                </msh:row><msh:row>
                <msh:textField property="medPolicySeries" size="10"/>
                <msh:textField property="medPolicyNumber" size="30"/>
            </msh:row><msh:row>
                <msh:autoComplete vocName="vocInsuranceCompanySmoCode" property="insuranceCompanyCode"
                ...
                <msh:row>
                    <msh:autoComplete property="ksg" vocName="vocKSGByBedSubType" parentId="bedSubType" fieldColSpan="6"
                                      horizontalFill="true"/>
                </msh:row>
                <msh:row>
                ...
                <msh:row>
                    <msh:autoComplete property="ksgPosition" vocName="vocKsgPositionByKsg" parentAutocomplete="ksg"
 ...
 where e.parententry_id=${param.id} and (e.isDeleted is null or e.isDeleted='0') order by e.startdate, e.starttime"/>
            <msh:table idField="1" name="childEntries" action="entityParentView-e2_entry.do"
                       noDataMessage="Нет дочерних СЛО">
...
where link.entry_id=${param.id}"/>
            <msh:table idField="1" name="patientDifficulty" action="/javascript:void()"
                       noDataMessage="Нет уровней сложности">
                <msh:tableColumn columnName="ИД" property="1"/>
                <msh:tableColumn columnName="Уровень сложности" property="2"/>
                <msh:tableColumn columnName="Коэффициент" property="3"/>
            </msh:table>
            <msh:section title="Случаи онкологического лечения"
                         createUrl="entityParentPrepareCreate-e2_cancerEntry.do?id=${param.id}">
...
where cancer.entry_id=${param.id}"/>
                <msh:table idField="1" deleteUrl="entityParentDeleteGoParentView-e2_cancerEntry.do" name="cancerEntry"
 ...
 where d.entry_id=${param.id}"/>
            ...
            <msh:section title="Особенности случая">
                <msh:autoComplete label="Добавить особенность" property="newFactor" vocName="vocE2EntryFactor"/><input
                    type="button" value="Д. фактор" onclick="addOrDeleteEntryFactor($('newFactor').value,false)">
        ...
        <msh:ifFormTypeAreViewOrEdit formName="e2_entryForm">
            ...
            <msh:ifFormTypeIsView formName="e2_entryForm">
                ...
                <script type="text/javascript">
                    ...
                    function splitLongCase() {
                        Expert2Service.splitLongCase(${param.id}, {
                            callback: function (a) {
                                alert(a);
                            }
                        });
            ...
            <msh:ifFormTypeIsNotView formName="e2_entryForm">
                <script type="text/javascript">
                    ...
                    ksgAutocomplete.addOnChangeCallback(function () {
                        $('isManualKsg').checked = true;
                        $('ksgPosition').value = "";
                        $('ksgPositionName').value = "";
                    });
                    ksgPositionAutocomplete.addOnChangeCallback(function () {
                        $('isManualKsg').checked = true;
                    });
                </script>
        ...
        <msh:ifFormTypeIsView formName="e2_entryForm">
            <msh:sideMenu>
                ...
                <msh:sideLink key="ALT+DEL" confirm="Удалить?" params="id"
                              ...
                              roles="/Policy/E2/Delete"/>
                <msh:sideLink action="/javascript:makeCheck()" name="Пересчитать случай" roles="/Policy/E2/Edit"/>
                <msh:sideLink action="/javascript:makeMPFromRecord()" name="Сделать файл из случая"
