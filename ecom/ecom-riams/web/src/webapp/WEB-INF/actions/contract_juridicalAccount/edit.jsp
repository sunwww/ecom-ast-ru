...
<tiles:insert page="/WEB-INF/tiles/main${param.short}Layout.jsp" flush="true">
	...
	<tiles:put name="body" type="string">
		<msh:form action="/entityParentSaveGoParentView-contract_juridicalAccount.do" defaultField="dateFrom" title="<a href='entityParentView-contract_juridicalAccount.do?id=${param.id}'>Счет</a>" >
			<msh:hidden property="id" />
			<msh:hidden property="saveType" />
			<msh:hidden property="contract" />
			<msh:panel colsWidth="1%,1%,1%">
				<msh:row>
					<msh:textField property="accountNumber" label="Номер счета"/>
					<msh:textField property="dateFrom" label="от"/>
				</msh:row>
				<msh:row>
					<msh:textField property="periodFrom" label="Период с"/>
		...
		<msh:ifFormTypeIsView formName="contract_juridicalAccountForm">
			<msh:section >
...
,round((cams.cost*(100-coalesce(mc.discountDefault,'0'))/100),0) as cost,cams.countMedService 
	...
	<tiles:put name="title" type="string">
		<ecom:titleTrail mainMenu="Contract" beginForm="contract_juridicalAccountForm" />
	</tiles:put>
	<tiles:put name="side" type="string">
		<msh:ifFormTypeAreViewOrEdit formName="contract_juridicalAccountForm">
			<msh:sideMenu>
				<msh:sideLink key="ALT+2" params="id" action="/entityParentEdit-contract_juridicalAccount" name="Изменить" title="Изменить" roles="/Policy/Mis/Contract/MedContract/ServedPerson/ContractAccount/Edit"/>
				<msh:sideLink key="ALT+DEL" confirm="Вы точно хотите удалить счет?" params="id" action="/entityParentDeleteGoParentView-contract_juridicalAccount" name="Удалить" title="Удалить" roles="/Policy/Mis/Contract/MedContract/ServedPerson/ContractAccount/Delete"/>
			</msh:sideMenu>
