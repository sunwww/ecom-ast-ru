...
<tiles:insert page="/WEB-INF/tiles/mainLayout.jsp" flush="true">
    ...
    <tiles:put name='side' type='string'>
        ...
        <msh:sideMenu title="Списки протоколов">
            <msh:sideLink key="ALT+2" styleId="${param.stAll}" params="id"
                          action="/printProtocolsBySLS.do?stAll=selected" name="все"/>
            <msh:sideLink key="ALT+3" styleId="${param.stNoPrint}" params="id"
                          action="/printProtocolsBySLS.do?stNoPrint=selected" name="нераспечатанные"/>
            <msh:sideLink key="ALT+4" styleId="${param.stPrint}" params="id"
                          action="/printProtocolsBySLS.do?stPrint=selected" name="распечатанные"/>
        </msh:sideMenu>
        <msh:sideMenu>
        </msh:sideMenu>
    ...
    <tiles:put name='body' type='string'>
        ...
        <msh:section>
            ...
            <msh:sectionContent>
                <ecom:webQuery name="protocols" nativeSql="select to_char(p.dateRegistration,'yyyymmdd') ||'!'||cast(p.timeRegistration as varchar(5)) ||'!'||p.id, p.dateRegistration, p.timeRegistration, p.record, p.printDate
                ...
                <msh:table selection="multiply" name="protocols" action="js-smo_visitProtocol-viewProtocol.do"
                           idField="1" noDataMessage="Нет протоколов">
                    <msh:tableNotEmpty>
                        <tr>
                            <th colspan='8'>
                                <msh:toolbar>
                                    <a href='javascript:printProtocols()'>Печать протоколов</a>
                                    <a href='entityParentPrepareCreate-smo_visitProtocol.do?id=${param.id }'>Добавить
                                        протокол</a>
                                </msh:toolbar>
                            </th>
                        </tr>
