...
, vbst.name as v8bstcode
 ...
 where wchb.visit_id =${param.id}
        ...
        -->
        <msh:form action="/entityParentSaveGoView-stac_sslAdmission.do" defaultField=""
                  title="Случай стационарного лечения. Поступление"
                  editRoles="/Policy/Mis/MedCase/Stac/Ssl/Admission/Edit"
                  createRoles="/Policy/Mis/MedCase/Stac/Ssl/Admission/Create">
            ...
            <msh:panel>
                ...
                <msh:row>
                    <td colspan="6">
                        <div style="display: none;" id='medPolicyInformation' class="errorMessage"/>
                    </td>
                </msh:row>
                ...
                <msh:row>
                    ...
                    <msh:ifFormTypeIsView formName="stac_sslAdmissionForm">
                        <msh:ifInRole roles="/Policy/Mis/MedCase/Stac/Ssl/Admission/ChangeStatCardNumber">
                            <td colspan="2">
                                <a id="changeStatCardNumber" href="javascript:changeStatCardNumber()">Изменить номер
                                    стат. карты</a>
                            </td>
                        </msh:ifInRole>
                    </msh:ifFormTypeIsView>
                ...
                <msh:row>
                    <msh:autoComplete property="lpu" label="Лечебное учреждение" vocName="lpu" fieldColSpan="3"
                                      horizontalFill="true"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete property="kinsman" label="Представитель"
                                      parentId="stac_sslAdmissionForm.patient" vocName="kinsmanBySMO"
                ...
                <msh:row>
                    <msh:autoComplete vocName="vocHospType" property="hospType" label="Тип тек. стационара"
                                      fieldColSpan="1" horizontalFill="true"/>
                    <msh:autoComplete vocName="vocServiceStream" property="serviceStream" label="Поток обслуживания"
                                      fieldColSpan="1" horizontalFill="true"/>
                </msh:row>
                <msh:ifInRole roles="/Policy/Mis/Contract/MedContract/ContractGuarantee/ContractGuaranteeLetter/View">
                    <msh:row>
                        <msh:autoComplete vocName="guaranteeByPatient" parentId="stac_sslAdmissionForm.patient"
                                          property="guarantee" label="Гарантийное письмо" fieldColSpan="3"
                                          horizontalFill="true"/>
                    </msh:row>
                </msh:ifInRole>
                <msh:row>
                    <msh:autoComplete property="pediculosis" vocName="vocPediculosis" label="Педикулез"
                                      horizontalFill="true"/>
                    <msh:autoComplete property="deniedHospitalizating" label="Причина отказа от госп."
                                      vocName="vocDeniedHospitalizating" horizontalFill="true" fieldColSpan="1"/>
                </msh:row>
                        ...
                        <td colspan="9">
                            <div id="formInfoMessage2" onclick="this.style.display = 'none'">
                                <table style="margin-left: 4em">
                                    <tr>
                                        <td>
                                            <div class="formInfoMessage">* При отказе от госпитализации не забудьте
                                                направить на амбулаторно-поликлиническое лечение!!!
                                            </div>
                                        </td>
                                    </tr>
                                </table>
                ...
                <msh:row>
                    <msh:textField property="attendant" label="Сопровождающее лицо" fieldColSpan="3"
                                   horizontalFill="true"/>
                </msh:row>
                <msh:row>
                ...
                <msh:row>
                    <msh:autoComplete vocName="vocHospTypeInAdmission" property="sourceHospType"
                                      label="Тип направившего ЛПУ" fieldColSpan="3" horizontalFill="true"/>
                </msh:row>
                <msh:row>
                ...
                <msh:row>
                    <msh:autoComplete property="orderMkb" label="Код МКБ направителя" vocName="vocIdc10AllPermitted"
                                      horizontalFill="true" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:textField property="orderDiagnos" label="ДИАГНОЗ напр. учреждения" fieldColSpan="3"
                                   horizontalFill="true"/>
                </msh:row>
                <msh:separator label="Доставлен (только для экстренных больных)" colSpan="9"/>
                <msh:row>
                    <msh:autoComplete showId="false" vocName="vocOmcFrm" hideLabel="false" property="orderType"
                                      viewOnlyField="false" horizontalFill="true"/>
                    <msh:textField property="supplyOrderNumber" label="Номер наряда"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete property="intoxication" label="Доставлен в сост. опьян.:"
                                      vocName="vocIntoxication" fieldColSpan="3" horizontalFill="true"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete property="preAdmissionDefect" label="Дефекты догосп. этапа"
                                      vocName="vocPreAdmissionDefect" fieldColSpan="3" horizontalFill="true"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete property="preAdmissionTime" label="Часы заболевания" vocName="vocPreAdmissionTime"
                                      horizontalFill="true"/>
                </msh:row>
                <msh:separator label="*Госпитализация" colSpan="9"/>
                <msh:row>
                    <msh:autoComplete property="department" label="Отделение" vocName="vocLpuHospOtd"
                                      horizontalFill="true" parentAutocomplete="lpu" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete property="ownerFunction" label="Деж. врач отд." vocName="workFunctionByLpu"
                                      horizontalFill="true" parentAutocomplete="department" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                ...
                <msh:ifFormTypeIsCreate formName="stac_sslAdmissionForm">
                    <msh:row>
                        <msh:autoComplete parentId="stac_sslAdmissionForm.patient" property="attachedPolicies"
                                          label="Прик.полис ОМС" horizontalFill="true" fieldColSpan="3"
                                          vocName="omcPolicyByPatient"/>
                    </msh:row>
                    <msh:row>
                        <msh:autoComplete parentId="stac_sslAdmissionForm.patient" property="attachedPolicyDmc"
                                          label="Прик.полис ДМС" horizontalFill="true" fieldColSpan="3"
                                          vocName="dmcPolicyByPatient"/>
                    </msh:row>
                </msh:ifFormTypeIsCreate>
                <mis:ifPatientIsWoman classByObject="Patient" idObject="stac_sslAdmissionForm.patient"
                                      roles="/Policy/Mis/Pregnancy/History/View">
                    <msh:separator label="Беременность" colSpan="9"/>
                    <msh:row>
                        <msh:autoComplete viewAction="entityParentView-preg_pregnancy.do" property="pregnancy"
                                          label="Беременность" fieldColSpan="3" parentId="stac_sslAdmissionForm.patient"
            ...
            <msh:panel>
                ...
                <msh:submitCancelButtonsRow functionSubmit="saveIdentityWithAsk();" colSpan="4"
