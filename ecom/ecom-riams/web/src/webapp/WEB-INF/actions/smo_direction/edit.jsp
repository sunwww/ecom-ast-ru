    ...
    <tiles:put name="body" type="string">
        <msh:form action="/entitySaveGoView-smo_direction" defaultField="orderLpuName">
            ...
            <msh:panel colsWidth="10%, 10%, 10%">
                <msh:row>
                    <td colspan="4">
                        <div id='medPolicyInformation' style="display: none;" class="errorMessage"/>
                    </td>
                </msh:row>
                <msh:row>
                ...
                <msh:row>
                    <msh:autoComplete showId="false" vocName="vocServiceStream" property="serviceStream"
                                      viewOnlyField="false" label="Поток обслуживания" fieldColSpan="3"
                                      horizontalFill="true"/>
                </msh:row>

                <msh:row>
                    <msh:autoComplete vocName="guaranteeByPatient" parentId="smo_directionForm.patient"
                                      property="guarantee" label="Гарантийное письмо" fieldColSpan="3"
                                      horizontalFill="true"/>
                </msh:row>
                ...
                <msh:ifFormTypeIsNotView formName="smo_directionForm">
                    ...
                    <msh:ifInRole roles="/Policy/Mis/MedCase/Direction/CreateNewTime">
                        <msh:row>
                            <msh:separator
                                    label="<a href='javascript:getWorkFunctionByUsername()'>Создание дополнительного времени</a>"
                                    colSpan="4"/>
                        </msh:row>
                        <msh:row>
                ...
                <msh:row>
                    <msh:autoComplete showId="false" vocName="vocWorkPlaceType" property="workPlaceType"
                                      viewOnlyField="false" label="Рабочее место" fieldColSpan="3"
                                      horizontalFill="true"/>
                </msh:row>
                <msh:row>
                ...
                <msh:row>
                    <msh:autoComplete showId="false" vocName="vocReason" property="visitReason" viewOnlyField="false"
                                      label="Цель визита" horizontalFill="true" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete property="kinsman" label="Представитель"
                                      parentId="smo_directionForm.patient" viewAction="entityParentView-mis_kinsman.do"
                                      vocName="kinsmanBySMO" horizontalFill="true" fieldColSpan="3"/>
                </msh:row>
                <msh:row>
                    <msh:autoComplete parentId="smo_directionForm.patient"
        ...
        <msh:ifFormTypeIsView formName="smo_directionForm">
            <msh:ifInRole roles="/Policy/Mis/MedCase/MedService/Create">
                <msh:section title="Лаб. услуги в ЛН">
                    <ecom:webQuery name="servicesLab"
                                   nativeSql="select
    	p.id as pid,ms.name as drname
 ,p.planStartDate,p.materialId,vpt.name as vptname
 ,coalesce(vpcr.name,'')||' '||coalesce(p.cancelReasonText,'') as fldCancel
 ,presV.datestart
,case when p.canceldate is not null then vwf.name||' '|| wp.lastname||' '||wp.firstname||' '||wp.middlename else null end as f13_cnsl
,case when p.canceldate is not null then 'color:red;' else null end as styleCancel
 from Prescription p
...
left join Patient as wp on wp.id=w.person_id
 where pl.medcase_id=${param.id} and p.DTYPE='ServicePrescription'
 and vms.code='LABSURVEY'
 order by p.planStartDate
      		"/>
                    <msh:table name="servicesLab" action="entityView-pres_servicePrescription.do"
                               idField="1"
                               styleRow="9">
        ...
        <msh:ifFormTypeIsNotView formName="smo_directionForm">
            <tags:smo_direction_time name="Time" workFuncId="workFunctionPlan" calenDayId="datePlan"
                                     calenTimeId="timePlan"/>
    ...
    <tiles:put name="side" type="string">
        <msh:ifFormTypeIsView formName="smo_directionForm">
            <msh:sideMenu title="Направление">
                <msh:sideLink key="ALT+2" params="id" action="/entityParentEdit-smo_direction" name="Изменить"
                              roles="/Policy/Mis/MedCase/Direction/Edit"/>
                <msh:sideLink key="ALT+DEL" confirm="Удалить?" params="id"
                              action="/entityParentDeleteGoParentView-smo_direction" name="Удалить"
                              roles="/Policy/Mis/MedCase/Direction/Delete"/>
            ...
            <msh:sideMenu title="Добавить">
                <msh:sideLink roles="/Policy/Mis/MedCase/Direction/Create" key="CTRL+1"
                              ...
                              title='Направление'/>
                <msh:sideLink roles="/Policy/Mis/MedCase/Direction/MedService/Create" key="ALT+8" name="Услугу"
                              params="id"
                              action="/entityParentPrepareCreate-smo_direction_medservice" title="Добавить услугу"/>
            </msh:sideMenu>
            <msh:sideMenu title="Печать">
                <msh:sideLink key="SHIFT+8" params="id"
                              action="/print-pat.do?s=PatientPrintService&amp;m=printInfoByMedcase"
                              name="Сведений о пациенте" title="Печать сведений о пациенте"
                              roles="/Policy/Mis/Patient/PrintInfoPatient"/>

                <msh:sideLink params="id" action="/print-vis_ticket.do?s=VisitPrintService&amp;m=printTalon1"
                              name="Талона" title="Печать талона" roles="/Policy/Mis/MedCase/Direction/Print"/>
                <msh:sideLink params="id" action="/print-vis_ticket2.do?s=VisitPrintService&amp;m=printTalon1"
                              name="Талона (верх. часть)" title="Печать талона (верх.часть)"
                              roles="/Policy/Mis/MedCase/Direction/Print"/>
                <msh:sideLink params="id" action="/print-vis_ticket1.do?s=VisitPrintService&amp;m=printTalon1"
                              name="Повторного талона" title="Печать повторного талона"
                              roles="/Policy/Mis/MedCase/Direction/Print1"/>
