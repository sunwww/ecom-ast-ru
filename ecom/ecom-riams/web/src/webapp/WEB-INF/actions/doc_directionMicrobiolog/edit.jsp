  ...
  <tiles:put name="body" type="string">
    <msh:form action="/entityParentSaveGoView-doc_directionMicrobiolog.do" defaultField="idc10" >
      ...
      <msh:panel>
      	...
      	<msh:row>
      		<msh:autoComplete property="materialAnalysis" horizontalFill="true" label="Материал" vocName="vocDocumentMaterialBiologAnalysis" fieldColSpan="8"/>
      	</msh:row>
      	<msh:row>
      		<msh:autoComplete property="analysis" horizontalFill="true" label="Исследование" vocName="vocDocumentBiologAnalysis" fieldColSpan="8"/>
      	</msh:row>
      	<msh:row>
      		<msh:autoComplete property="objectAnalysis" horizontalFill="true" label="Цель" vocName="vocDocumentObjectBiologAnalysis" fieldColSpan="8"/>
      	</msh:row>
  ...
  <script type="text/javascript">
    function printDocument() {
      	if (confirm('Распечатать документ?')) {
      		window.location.href = "print-documentDirectMicrobiolog.do?next=entityParentView-smo_visit.do__id="+$('medCase').value+"&s=VisitPrintService&m=printDocument&id=${param.id}" ;
      	}
  }
    printDocument();
    </script>
