...
public class TicketServiceBean implements ITicketService {
    ...
    public void unionSpos(Long aOldSpo, Long aNewSpo) {
        List<ShortMedCase> list = manager.createQuery("from ShortMedCase where parent_id=:oldSpo").setParameter("oldSpo", aOldSpo).getResultList();
        PolyclinicMedCase spoOld = manager.find(PolyclinicMedCase.class, aOldSpo);

        if (!list.isEmpty() && spoOld != null) {
            PolyclinicMedCase spoNew = manager.find(PolyclinicMedCase.class, aNewSpo);
            ...
            manager.persist(spoNew);
            manager.remove(spoOld);
        }
    }

    public void moveVisitInOtherSpo(Long aVisit, Long aNewSpo) {
        List<Object[]> list = manager.createNativeQuery("select max(spo.id) as spo,count(allv.id) as allvid" +
        ...
        ShortMedCase vis = manager.find(ShortMedCase.class, aVisit);
        ...
        if (!list.isEmpty() && vis != null) {
            ...
            if (aNewSpo != null && !aNewSpo.equals(0L)) {
                spoNew = manager.find(PolyclinicMedCase.class, aNewSpo);
                if (spoNew == null) throw new IllegalDataException("Неправильно определен СПО");
            } else {
            ...
            if (spo != null && vis.getParent().getId() != spo && (cnt == null || cnt.intValue() < 2)) {
                ...
                PolyclinicMedCase spoOld = manager.find(PolyclinicMedCase.class, spo);
    ...
    public String getMedServiceBySpec(Long aSpec, String aDate) throws ParseException {
        ...
        LOG.info(sqlMain);
        List<Object[]> list = manager.createNativeQuery(sqlMain.toString())
                .getResultList();
        if (!list.isEmpty()) {
            try {
    ...
    public String findDoubleBySpecAndDate(Long aId, Long aMedcard, Long aSpecialist, String aDate) {
        ...
        List<Object[]> doubles = manager.createNativeQuery(sql.toString())
    ...
    public List<TicketForm> findActiveMedcardTickets(Long aMedcard) {
        ...
        Query query = builder.build(manager, "from Ticket where status=" + Ticket.STATUS_OPEN, " order by date, time");
    ...
    public List<TicketForm> findStatTicketByDateAndUsername(String aDateInfo, String aDate, String aUsername) {
        ...
        Query query = builder.build(manager, "from Ticket where "
    ...
    public List<TicketForm> findOpenTicketByDate(String aDate) {
        ...
        Query query = builder.build(manager, "from Ticket where status<>" + Ticket.STATUS_CLOSED, " order by date,time, status");
    ...
    public List<TicketForm> findTicketsByNumber(String aNumber) {
        ...
        Query query = builder.build(manager, "from Ticket where id ='" + aNumber + "'", " order by date, time");
        return createList(query);
    }
    ...
    public List<TicketForm> findAllMedcardTickets(Long aMedcard) {
        QueryClauseBuilder builder = new QueryClauseBuilder();
        builder.add("medcard_id", aMedcard);
        Query query = builder.build(manager, "from Ticket where", " order by date");
        return createList(query);
    }
    ...
    public List<TicketForm> findAllSpecialistTickets(Long aSpecialist) {
        ...
        Query query = builder.build(manager, "from Ticket where", " order by date, status");
        return createList(query);
    }
    ...
    public List<TicketForm> findAllWorkerTickets(Long aSpecialist, String aDate, int aStatus) {
        QueryClauseBuilder builder = new QueryClauseBuilder();
        Object obj = manager.createNativeQuery("select list(wf1.id) from WorkFunction wf left join Worker w on w.id=wf.worker_id left join WorkFunction wf1 on wf1.worker_id=wf.worker_id where wf.id=" + aSpecialist).getSingleResult();
        ...
        Query query = builder.build(manager, "from Ticket where workFunction_id in (" + obj + ") " + stat, " order by date,time, status");
        return createList(query);
    }
    ...
    public List<TicketForm> findAllSpecialistTickets(Long aSpecialist, String aDate, int aStatus) {
        ...
        if (date != null) builder.add("date", date);
        Query query = builder.build(manager, "from Ticket where " + stat, " order by date,time, status");
        LOG.info("Запрос по ticket: ");
        LOG.info(query.toString());
        return createList(query);
    }
    ...
    public void closeTicket(Long aTicketId) {
        Ticket ticket = manager.find(Ticket.class, aTicketId);
    ...
    private List<TicketForm> createList(Query aQuery) {
        ...
        for (Ticket ticket : list) {
            try {
                ret.add(entityFormService.loadForm(TicketForm.class, ticket));
