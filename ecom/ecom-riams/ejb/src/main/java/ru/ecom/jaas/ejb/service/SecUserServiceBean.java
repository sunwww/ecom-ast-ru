...
public class SecUserServiceBean implements ISecUserService {
    ...
    private void createPersonalWorkFunction(SecUser secUser, Worker w, VocWorkFunction vocWorkFunction, Patient patient, String username, java.sql.Date dd, java.sql.Time tt, Boolean isAdmin) {
        ...
        if (isAdmin) {
            ...
            manager.createNativeQuery("update workfunction set isadministrator=false" +
                    ...
                    " where wf.id=" + newId + " limit 1)" +
                    " where w.lpu_id =wnow.lpu_id and w.id<>wnow.id and wf.id<>" + newId + ")")
                    .executeUpdate();
        }
     ...
     * @return Сообщение
     */
    private String addWorkFunctionToUser(SecUser secUser, MisLpu misLpu, VocWorkFunction vocWorkFunction, String newPsw, Long userCopyId, Patient patient, java.sql.Date dd, java.sql.Time tt, Boolean isAdmin) throws IOException {
        String username = context.getCallerPrincipal().getName();
        String msgResult = "";
        if (misLpu != null && vocWorkFunction != null) {
            if (secUser != null) {
                ...
                if (wfId == 0L) {
                    createPersonalWorkFunction(secUser, w, vocWorkFunction, patient, username, dd, tt, isAdmin);
    ...
    public String addUserToHospShort(Long aPatientId, Long aLpuId, Long avWfId, String newPsw, Long userCopyId, String username, Long aUserId, Boolean isAdmin) throws IOException {
        ...
        if (aUserId != null) {
            ...
            return secUser != null ?
                    addWorkFunctionToUser(secUser, misLpu, vocWorkFunction, newPsw, userCopyId, patient, dd, tt, isAdmin) :
            ...
            return addWorkFunctionToUser(secUser, misLpu, vocWorkFunction, newPsw, userCopyId, patient, dd, tt, isAdmin);
        } else
