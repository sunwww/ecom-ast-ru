...
public class QualityEstimationServiceBean implements IQualityEstimationService {
    ...
    public QualityEstimationRow getRow(Boolean aFullExpertCard, String aRequestParam, Long aCardId, String aTypeSpecialist, boolean aView) {
        ...
        boolean ifTypeBool = false, isKmp = false, isNot203 = false;
        String sql2 = "select case when vqec.code='PR203' then '1' " +
                "else case when vqec.code='KMP' then '-1' " +
                "else case when vqec.code='NOT 203' then '2' else '0' end end end " +
                "from QualityEstimationCard qec " +
                "left join VocQualityEstimationKind vqec on vqec.id=qec.kind_id " +
                "where qec.id=:cardId";
        List<Object[]> listkind = manager.createNativeQuery(sql2).setParameter("cardId", aCardId).getResultList();
        if (!listkind.isEmpty()) {
            Object one = "1", _one = "-1", two = "2";
        ...
        if (ifTypeBool || isNot203) table.append("<th>Комментарии</th>");
        table.append("</tr>");
        table.append("<tr>");
        table.append("<th>зав. отд</th>");
        table.append("<th>эксперт (КР)</th>");
        if (ifTypeBool || isNot203) table.append("<th>Комм. зав.</th>");
        table.append("</tr>");
        List<Object[]> list = manager.createNativeQuery(sql.toString()).getResultList();
        if (!list.isEmpty()) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (cntSubsection < 1) {
                    ...
                    int tmp = (ifTypeBool || isNot203) ? 5 : 4;
    ...
    public QualityEstimationRow getRowShort(boolean aReplaceValue, HashMap<String, String> aValueMap, Long aKind, Long aCard, String aTypeSpecialist, int aCntSection, boolean aView
        ...
        sql.append("select distinct vqec.id as vqecid,vqec.code as vqeccode,vqec.name as vqecname ")
                ...
                .append(" left join vocqualityestimationcrit_diagnosis vqecrit_d on vqecrit_d.vqecrit_id=vqec.id");
        boolean ifTypeBool = false, isNot203 = false;
        String sql2 = "select case when vqec.code='PR203' then '1' " +
                "else case when vqec.code='KMP' then '-1' " +
                "else case when vqec.code='NOT 203' then '2' else '0' end end end " +
                "from QualityEstimationCard qec " +
        ...
        if (!listkind.isEmpty()) {
            Object one = "1", _one = "-1", two = "2", zero = "0";
            Object row = listkind.get(0);
            if (!row.equals(zero)) {
                if (row.equals(one))
                    ifTypeBool = true;
        ...
        if (ids == null || ids.isEmpty()) {
            ...
            if (mcase != null && kindId != null && wf != null && idc10 != null && cardnumber != null) {
                ...
                sql.append("select distinct vqecrit.id")
                        ...
                        .append(" and kind.code='").append(aCode).append("'");
                if (!"NOT 203".equals(aCode))
                    sql.append(" and reg.code='4'");
                ...
                if (crits != null) {
                    for (Object critO : crits) {
                        ...
                        Long id = Long.parseLong(critO.toString());
