...
public class NewBornPreCreateInterceptor implements IParentFormInterceptor {
    ...
    public void intercept(IEntityForm aForm, Object aEntity, Object aParentId, InterceptorContext aContext) {
        ...
        if (form instanceof NeonatalNewBornForm) {
            ...
            //проверка на то, что текущее родовое отделение - последнее в СЛС
            List<BigInteger> listCheck = aContext.getEntityManager().createNativeQuery("select max(slo.id) from medcase slo" +
                    " left join medcase sls on sls.id=slo.parent_id" +
                    " left join medcase m on m.parent_id=sls.id" +
                    " left join mislpu dep on dep.id=slo.department_id" +
                    " where dep.ismaternityward and m.id=:parent").setParameter("parent", aParentId).getResultList();
            if (!listCheck.isEmpty()) {
                String res = listCheck.get(0).toString();
                String parent = aParentId.toString();
                if (res != null && !res.equals(parent))
                    throw new IllegalStateException("Новорождённого обязательно создавать в последнем родовом отделении в истории пациентки!" +
                            "<a href='entityParentView-stac_slo.do?id=" + res + "'>Перейти к нему</a>");
            }
