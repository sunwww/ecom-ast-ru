...
public class PatientServiceBean implements IPatientService {
    ...
    public VocSex getSexByName(String firstname) {
        try {
            return ((VocSexName) manager.createNamedQuery("VocSexName.getByName")
     ...
     */
    public String exportDispensaryCard(java.util.Date dateFrom, java.util.Date dateTo, java.util.Date dateChanged, String packetNumber) {
        ...
        try {
            ...
            if (dateFrom != null) {
                ...
                if (dateTo != null) {
                    sqlWhere.append(" between '").append(yyyyMMdd.format(dateFrom)).append("' and '").append(yyyyMMdd.format(dateTo)).append("'");
                } else {
                    sqlWhere.append(">='").append(yyyyMMdd.format(dateFrom)).append("'");
                }
                if (dateChanged != null) {
                    sqlWhere.append(" and coalesce(d.editDate, d.createDate)>='").append(yyyyMMdd.format(dateChanged)).append("'");
                }
            }
            while (packetNumber.length() < 4) {
            ...
            JSONArray arr = new JSONArray(webQueryService.executeNativeSqlGetJSON(flds, sql, null));
            ...
            filename += ".xml";
            if (arr.length() == 0) {
                ret.put("status", "error").put("errorCode", "Записей не найдено");
                return ret.toString();
            }
            Element zap = null;
            ...
            for (int i = 0; i < arr.length(); i++) {
                ...
                if (lastPatId == null || !lastPatId.equals(patId)) {
                    zap = new Element("ZAP");
                    addElement(zap, "N_ZAP", patId);
                    addElement(zap, "YEAR", pat.getString("YEAR"));
                    addElement(zap, "FAM", pat.getString("FAM"));
                    addElement(zap, "IM", pat.getString("IM"));
                    addElement(zap, "OT", pat.getString("OT"));
                    addElement(zap, "DR", pat.getString("DR"));
                    addElement(zap, "TEL", pat.getString("TEL"));
                    dn.addContent(zap);
                    lastPatId = patId;
                }

                Element zapDn = new Element("DN");
                addElement(zapDn, "IDCASE", pat.getString("IDCASE"));
                addElement(zapDn, "PROFIL", pat.getString("PROFIL"));
                addElement(zapDn, "DS", pat.getString("DS"));
                addElement(zapDn, "D_BEG", pat.getString("D_BEG"));
                addElement(zapDn, "D_END", pat.getString("D_END"));
                addElement(zapDn, "END_RES", pat.getString("END_RES"));
    ...
    private void addElement(Element element, String fldName, String fldValue) {
        element.addContent(new Element(fldName).setText(fldValue));
    }

    public void changeMedPolicyType(Long aPolicyId, Long aNewPolicyTypeId) {
        manager.createNativeQuery("update medpolicy set dtype=(select vmp.code from vocmedpolicy vmp" +
    ...
    public boolean needUpdatePatient(Long aPatientId, Long aPatientFondId) {
        ...
        PatientFond fond = manager.find(PatientFond.class, aPatientFondId);
        Patient pat = manager.find(Patient.class, aPatientId);
        ...
        if (aLpuAttached != null && aAttachedDate != null && aAttachedType != null) {
            ...
            Object a = manager.createNativeQuery(aSql).getSingleResult();
        ...
        if (aPolicyNumber != null && aPolicySeries != null && aCompanyCode != null) {
            Object mp = manager.createNativeQuery("select count(mp.id) from medpolicy mp" +
    ...
    private boolean needChangeValue(Object aOldValue, Object aNewValue) {
        if (toStr(aOldValue) == null && toStr(aNewValue) != null) {
            return true;
        } //Старое значение пустое, новое - нет, обновляем.
        if (toStr(aNewValue) == null) {
            return false;
        } //На пустое значение не обновляем.
    ...
    public void insertExternalDocumentByObject(String aObject, Long aObjectId, Long aType, String aReferenceComp, String aReferenceTo, String aComment, String aUsername) {
        ...
        switch (aObject) {
            case "Patient":
                Patient pat = manager.find(Patient.class, aObjectId);
                doc.setPatient(pat);
                break;
            case "MedCase":
                MedCase mc = manager.find(MedCase.class, aObjectId);
                doc.setMedCase(mc);
                break;
            case "Template":
                break;
        ...
        if (aType != null && aType > 0) {
            VocExternalDocumentType type = manager.find(VocExternalDocumentType.class, aType);
            doc.setType(type);
        }
        manager.persist(doc);
    }
    ...
    public void createNaturalPerson(Long aPatient) {
        Patient pat = manager.find(Patient.class, aPatient);
    ...
    public String getAddressByOkato(String aOkato, String aStreet) {
        if (isNullOrEmpty(aOkato) || isNullOrEmpty(aStreet)) return null;
        String streetType = "";
        if (aOkato.length() < 11) {
            aOkato += "0000000000";
        ...
        List<Object> listO = manager.createNativeQuery(sql.toString()).setMaxResults(10).getResultList();
        if (!listO.isEmpty()) {
            if (listO.size() > 1) {
    ...
    private String getAddressByKladr(String aKladr, String aRegion, String aRayon, String aCity, String aStreet, String aOkato) {
        ...
        List<Object[]> list = manager.createNativeQuery(sql.toString()).setMaxResults(1).getResultList();
        if (!list.isEmpty()) {
            res.append(list.get(0)[0]);
        } else {
            if (aStreet.startsWith("?")) return res.toString();
            if (!isNullOrEmpty(aKladr)) {
    ...
    public String getInfoVocForFond(String aPassportType, String aAddress, String aPolicy) {
        ...
        JSONObject ret = new JSONObject();
        if (!isNullOrEmpty(aPassportType)) {
    ...
    public boolean updateDataByFondAutomatic(Long aPatientFondId, Long aCheckId
        ...
        try {
            if (aPatientFondId != null && (needUpdateAttachment || needUpdateDocuments
                ...
                PatientFond patF = manager.find(PatientFond.class, aPatientFondId);
                String rz = patF.getCommonNumber(), aCompany = patF.getCompanyCode(), doctorSnils = patF.getDoctorSnils();
                ...
                if (needUpdatePatient) {
                    ...
                    if (address != null) {
                        patient.setAddress(manager.find(Address.class, Long.parseLong(address)));
                ...
                if (needUpdateDocuments) {
                    ...
                    if (!isNullOrEmpty(docType) && !isNullOrEmpty(docNumber)) {
                        ...
                        List<VocIdentityCard> cards = manager.createNamedQuery("VocIdentityCard.findByOmcCode")
                ...
                if (needUpdateAttachment) {
                    ...
                    patF.setIsAttachmentUpdate(false);
                }
                manager.persist(patF);
    ...
    public String updateOrCreateAttachment(Long aPatientId, String aCompany, String aLpu, String aAttachedType, String aAttachedDate, String aDoctorSnils
        ...
        List<RegInsuranceCompany> companies = (List<RegInsuranceCompany>) manager.createQuery(sqll)
        ...
        if (sc != null && sc.getKeyValue().equals(aLpu) && insCompany != null) { //Создаем прикрепления только своей ЛПУ
            ...
            if (aDoctorSnils != null && !aDoctorSnils.trim().equals("")) { // ищем участок по СНИЛС врача
                obj = manager.createNativeQuery("select la.id " +
                        " from lpuarea la" +
            ...
            if ("1".equals(aAttachedType)) {
                obj = null;
                try {
                    obj = manager.createNativeQuery("select la.id from patient p" +
            ...
            if (areaId != null) {
                la = manager.find(LpuArea.class, areaId);
            }
            List<LpuAttachedByDepartment> attachments = manager.createQuery("from LpuAttachedByDepartment where patient_id=:pat and dateTo is null")
                    .setParameter("pat", aPatientId).getResultList();
            List<VocAttachedType> attachedTypeList = manager.createQuery("from VocAttachedType where code=:code").setParameter("code", aAttachedType).getResultList();


            if (attachedTypeList.isEmpty()) {
                return "Прикрепление не создано, не распознан тип прикрепления: " + aAttachedType;
            }
            ...
            if (attachments.isEmpty()) { // Создаем новое
                ...
                if (la != null) {
                    ...
                    Long l = Long.valueOf(manager.createNativeQuery("select keyvalue from SoftConfig sc where sc.key='DEFAULT_LPU'").getResultList().get(0).toString());
                    lpuAtt = manager.find(MisLpu.class, l);
                }
                if (lpuAtt != null) {
                    LpuAttachedByDepartment att = new LpuAttachedByDepartment();
                    att.setPatient(manager.find(Patient.class, aPatientId));
                    att.setLpu(lpuAtt);
                    att.setAttachedType(attType);
                    att.setArea(la);

                    try {
                        att.setDateFrom(DateFormat.parseSqlDate(aAttachedDate));
                        ...
                        LOG.error("Дата не распознана " + aAttachedDate);
                        e.printStackTrace();
                    }
                    ...
                    ret.append("0ЛПУ с кодом '").append(aLpu).append("' не найдено, прикрепление не создано. ");
                }
                ...
                for (LpuAttachedByDepartment a : attachments) {
                    ...
                    ret.append("Обновлено прикрепление. Старый тип - ").append(a.getAttachedType() != null ? a.getAttachedType().getCode() : "").append(", дата - ").append(DateFormat.formatToDate(a.getDateFrom())).append(". Новый тип - ").append(attType.getCode()).append(", дата - ").append(aAttachedDate).append(". ");
                    str.append("update LpuAttachedByDepartment set ").append(updateDate).append(" dateFrom=to_date('").append(aAttachedDate).append("','dd.mm.yyyy'), company_id='").append(insCompany.getId()).append("', editusername='fond_check', attachedtype_id=").append(attType.getId()).append(" ").append(areaSql).append(" where id='").append(a.getId()).append("'");
                    manager.createNativeQuery(str.toString()).executeUpdate();
                }
            }
        }
    ...
    public boolean updateDataByFond(String aUsername, Long aPatientId, String aFiodr
        ...
        if (aIsAddress && aAddress != null && !aAddress.equals("") && aAddress.split("#").length > 8) {
            String[] adr = aAddress.split("#");
            StringBuilder sql;
            String kladr = adr[0];
            String city = adr[5].toUpperCase();
            ...
            String addressid = getAddressByKladr(kladr, region, rayon, city, street, okato);

            sql = new StringBuilder();
            sql.append("update Patient set ");
            if (!isNullOrEmpty(addressid)) sql.append(" address_addressid='").append(addressid).append("' , ");
    ...
    public boolean updateOrCreatePolicyByFond(Long patientId, String rz, String lastname, String firstname
            , String middlename, String birthday, String companyCode, String policySeries
            , String policyNumber, String actualDateFrom, String actualDateTo, String currentDate) {
        StringBuilder sql = new StringBuilder();
        sql.append("select count(*) from medpolicy ");
        sql.append(" where patient_id='").append(patientId).append("'");
        sql.append(" and series='").append(policySeries).append("'");
        sql.append(" and polNumber='").append(policyNumber).append("'");
        Object cnt = manager.createNativeQuery(sql.toString()).getSingleResult();
        Long cntL = ConvertSql.parseLong(cnt);

        String type = getTypePolicy(policySeries);
        sql = new StringBuilder();
        if (cntL != null && cntL > 0L) {
            sql.append("update MedPolicy set lastname='").append(lastname).append("'");
            sql.append(", firstname='").append(firstname).append("'");
            sql.append(", middlename='").append(middlename).append("'");
            sql.append(", birthday=to_date('").append(birthday).append("','dd.mm.yyyy')");
            sql.append(", company_id=(select id from REG_IC where omcCode='").append(companyCode).append("')");
            sql.append(", actualDateFrom=to_date('").append(actualDateFrom).append("','dd.mm.yyyy')");
            if (actualDateTo != null && !actualDateTo.equals("")) {
                sql.append(", actualDateTo=to_date('").append(actualDateTo).append("','dd.mm.yyyy')");
            } else {
                sql.append(", actualDateTo=null");
            }
            sql.append(", commonNumber='").append(rz).append("'");
            sql.append(", type_id=(select id from VocMedPolicyOmc where code='").append(type).append("')");
            sql.append(", confirmationDate=to_date('").append(currentDate).append("','dd.mm.yyyy')");
            sql.append(" where patient_id='").append(patientId).append("'");
            sql.append(" and series='").append(policySeries).append("'");
            sql.append(" and polNumber='").append(policyNumber).append("'");
            ...
            manager.createNativeQuery(sql.toString()).executeUpdate();
            try {
                Date dat = DateFormat.parseSqlDate(actualDateFrom);
                ...
                sql = new StringBuilder();
                sql.append("update medpolicy set actualdateto=(to_date('").append(dateFrom).append("','dd.mm.yyyy')) where patient_id='").append(patientId)
                        .append("' and actualdateFrom<to_date('").append(actualDateFrom)
                        .append("','dd.mm.yyyy') and (actualdateTo>=to_date('").append(actualDateFrom)
            ...
            sql.append("select count(*) from medpolicy ");
            sql.append(" where series='").append(policySeries).append("'");
            sql.append(" and polNumber='").append(policyNumber).append("'");
            Object cnt1 = manager.createNativeQuery(sql.toString()).getSingleResult();
            Long cntL1 = ConvertSql.parseLong(cnt1);
            if (cntL1.equals(0L)) {
                sql = new StringBuilder();
                sql.append("select id,omcCode from REG_IC where omcCode='").append(companyCode).append("' order by id desc");
                List<Object[]> idS = manager.createNativeQuery(sql.toString()).setMaxResults(1).getResultList();
                sql = new StringBuilder();
                sql.append("select id,code from VocMedPolicyOmc where code='").append(type).append("' order by id desc");
                List<Object[]> idT = manager.createNativeQuery(sql.toString()).setMaxResults(1).getResultList();
                if (!idS.isEmpty() && !idT.isEmpty()) {
                    ...
                    sql.append(", to_date('").append(actualDateFrom).append("','dd.mm.yyyy')");

                    if (actualDateTo != null && !actualDateTo.equals("")) {
                        sql.append(", to_date('").append(actualDateTo).append("','dd.mm.yyyy')");
                    } else {
                        sql.append(", null");
                    }
                    //sql.append(", to_date('").append(pol[4]).append("','dd.mm.yyyy')") ;
                    sql.append(", '").append(rz).append("'");
                    sql.append(", '").append(patientId).append("'");
                    sql.append(", '").append(policySeries).append("'");
                    sql.append(", '").append(policyNumber).append("','").append(idT.get(0)[0]).append("','").append(lastname).append("'");
                    sql.append(",'").append(firstname).append("'");
                    sql.append(",'").append(middlename).append("'");
                    sql.append(", to_date('").append(birthday).append("','dd.mm.yyyy'),to_date('").append(currentDate).append("','dd.mm.yyyy'))");
                    manager.createNativeQuery(sql.toString()).executeUpdate();
                    ...
                    try {
                        ...
                        String dateFrom = format.format(cal.getTime());
                        sql = new StringBuilder();
                        sql.append("update medpolicy set actualdateto=(to_date('").append(dateFrom).append("','dd.mm.yyyy')-1) where patient_id='").append(patientId)
                                .append("' and actualdateFrom<to_date('").append(actualDateFrom)
                                .append("','dd.mm.yyyy') and (actualdateTo>=to_date('").append(actualDateFrom)
                                .append("','dd.mm.yyyy') or actualdateTo is null)");
                        manager.createNativeQuery(sql.toString()).executeUpdate();
    ...
    public PatientForm getPatientById(Long aId) {
        ...
        String sql = "select p.lastname,p.firstname,p.middlename,to_char(p.birthday,'dd.mm.yyyy') as birthday,p.snils " +
                ...
                " ,mp.series,mp.polNumber, ri.omcCode as riomccode,mp.commonNumber,a.kladr" +
                " ,p.houseNumber,p.houseBuilding,flatNumber" +
                " from patient p " +
                ...
                " where p.id='" + aId + "' order by mp.actualDateFrom desc";
        List<Object[]> list = manager.createNativeQuery(sql).setMaxResults(1).getResultList();
        if (!list.isEmpty()) {
    ...
    public String getOmcCodeByPassportType(Long aPassportType) {
        VocIdentityCard vic = manager.find(VocIdentityCard.class, aPassportType);
        return vic != null ? vic.getOmcCode() : "";
    }

    public void setAddParamByMedCase(String aParam, Long aMedCase, Long aStatus) {
        MedCase mc = manager.find(MedCase.class, aMedCase);
    ...
    public void movePatientDoubleData(Long aIdNew, Long aIdOld) {
        Patient newpat = manager.find(Patient.class, aIdNew);
        Patient oldpat = manager.find(Patient.class, aIdOld);
        if (!newpat.getSex().getOmcCode().equals(oldpat.getSex().getOmcCode()))
            throw new IllegalArgumentException("Нельзя данные перенести персонам разных полов!!!");
        manager.createNativeQuery("	update Patient set attachedOmcPolicy_id = null where id =:idold	").setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update DeathCase set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update DeathCase set mother_id =:idnew where mother_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update Diagnosis set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update DisabilityCase set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update DisabilityDocument set nursedPatient_id =:idnew where nursedPatient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update DisabilityDocument set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        ...
        manager.createNativeQuery("	update Kinsman set kinsman_id =:idnew where kinsman_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update LpuAttachedByDepartment set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update ExtDispPlanPopulationRecord set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update Medcard set person_id =:idnew where person_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update MedCase set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update MedPolicy set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update NewBorn set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update Pregnancy set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update Privilege set person_id =:idnew where person_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update SurgicalOperation set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update Vaccination set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update Worker set person_id =:idnew where person_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update Kinsman set person_id =:idnew where person_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        ...
        manager.createNativeQuery("	update ExtDispCard set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("	update ClinicExpertCard set patient_id =:idnew where patient_id =:idold	").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("  update PatientFond set patient =:idnew where patient =:idold ").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("  update JournalPatientFondCheck set patient_id =:idnew where patient_id =:idold ").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("  update JournalChangePatient set patient_id =:idnew where patient_id =:idold ").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("  update SuicideMessage set patient_id =:idnew where patient_id =:idold ").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("  update QualityEstimationCard set patient_id =:idnew where patient_id =:idold ").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("  update Covid19 set patient_id =:idnew where patient_id =:idold ").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
        manager.createNativeQuery("  update observationsheet set patient_id =:idnew where patient_id =:idold ").setParameter("idnew", aIdNew).setParameter("idold", aIdOld).executeUpdate();
    }
    ...
    public List<VocOrgForm> findOrg(String aNewNumber, String aOldNumber, String aName) {
        ...
        boolean where = false;
        if (!isNullOrEmpty(aOldNumber)) {
            where = true;
            sql.append(" where ");
            sql.append(" oldFondNumber like '%").append(aOldNumber).append("%'");
        }
        if (!isNullOrEmpty(aNewNumber)) {
            if (where) {
                sql.append(" and ");
            } else {
                sql.append(" where ");
        ...
        List<VocOrg> orgs = manager.createQuery(sql.toString())
                .setMaxResults(50).getResultList();
        for (VocOrg org : orgs) {
            try {
                ret.add(entityFormService.loadForm(VocOrgForm.class,
                        org));
    ...
    public WebQueryResult findPatient(Long aLpuId, Long aLpuAreaId,
        ...
        if (isEnableLimitAreas) {
            ...
            if (!isNullOrEmpty(aLastname)) {
                ...
                sqlFld.append(" where pf1.id=(select max(pf.id) from PatientFond pf where pf.lastname=p.lastname and pf.firstname=p.firstname and pf.middlename=p.middlename and pf.birthday=p.birthday ) ");
                sqlFld.append(" ) as fondinfo ");
                Query query1 = builder.buildNative(manager, sqlFld +
                        " from JournalChangePatient jcp " +
                        " left join patient p on jcp.patient_id=p.id " +
                        sql +
                        " and " + p2 + " and (jcp.lastname!=p.lastname or jcp.firstname!=p.firstname or jcp.middlename!=p.middlename)", " group by p.id,p.lastname,p.firstname,p.middlename,p.birthday,p.patientSync,p.ColorType order by  p.lastname " + (aNext ? "" : " desc") + ",p.firstname " + (aNext ? "" : " desc") + ",p.middlename " + (aNext ? "" : " desc") + ",p.id " + (aNext ? "" : " desc"));
            ...
            Query query = builder.buildNative(manager, sqlFld + " from patient p " + sql + p1,
            ...
            String sqlFld1 = "select p.id,p.lastname||' '||list(case when p.lastname!=jcp.lastname then '('||jcp.lastname||')' else '' end) as lastname" +
                    ...
                    " ) as fondinfo ";
            Query query1 = builder.buildNative(manager, sqlFld1 +
                            " from JournalChangePatient jcp " +
                            " left join patient p on jcp.patient_id=p.id " +
                            sql +
                            p2 +
                            " and (jcp.lastname!=p.lastname or jcp.firstname!=p.firstname or jcp.middlename!=p.middlename)" +
                            " and not (" + p1 + ")",
                    "group by p.id,p.lastname,p.firstname,p.middlename,p.birthday ,p.patientSync, p.colorType order by  p.lastname " + (aNext ? "" : " desc") + ",p.firstname " + (aNext ? "" : " desc") + ",p.middlename " + (aNext ? "" : " desc") + ",p.id " + (aNext ? "" : " desc"));
            ...
            // поиск по полису
            if (!isNullOrEmpty(aLastname) && (ret1.isEmpty() && ret2.isEmpty())) {
            ...
            if (!isNullOrEmpty(aLastname) && (ret1.isEmpty() && ret2.isEmpty() && ret3.isEmpty())) {
                appendNativeToList(findByPolicy(sqlFld, aLastname, aYear), ret3, null, true);
            }
        }
        wqr.set1(ret1);
        wqr.set2(ret2);
    ...
    private Query findByPolicy(StringBuilder aSqlFld, String aPolicyQuery, String aYear) {
        ...
        String sql = aSqlFld +
                ...
                " where";
        return b.buildNative(manager, sql, "group by p.id, p.id,p.lastname,p.firstname,p.middlename,p.birthday,p.patientSync, p.colorType order by p.lastname, p.firstname,p.middlename,p.id");//order by MedPolicy.patient.lastname, MedPolicy.patient.firstname");
    }
    ...
    private Query findByPatientRz(StringBuilder aSqlFld, String aPolicyQuery, String aYear) {
        ...
        String sql = aSqlFld +
                ...
                " left join VocAttachedType vat on vat.id=att.AttachedType_id" +
                " where";
        return b.buildNative(manager, sql
                , "group by p.id, p.id,p.lastname,p.firstname,p.middlename,p.birthday,p.patientSync, p.colorType order by p.lastname, p.firstname,p.middlename,p.id");
    }
    ...
    private void appendNativeToList(Query aQuery, List<PatientForm> ret, String aAddInfo, boolean aNext) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (arr.length > 7 && arr[7] != null) {
                f.setLpuName((String) arr[7]);
            }
            if (arr.length > 8 && arr[8] != null) {
                f.setLpuAreaName((String) arr[8]);
            }
            if (arr.length > 9 && arr[9] != null) {
                f.setPatientInfo((String) arr[9]);
            }
            ret.add(f);
        }
    ...
    public String findPatientLpuInfo(Long aAddressId, String aNumber,
        ...
        Address address = manager.find(Address.class, aAddressId);
    ...
    public LpuAreaAddressPoint findPoint(Address aAddress, String aNumber,
        ...
        // дом корпус кв
        if (point == null && !isNullOrEmpty(aNumber) && !isNullOrEmpty(aBuilding)
                && !isNullOrEmpty(aFlat)) {
            point = findPoint(aAddress, aNumber, aBuilding, aBirthday, null);
            // дом кв
        } else if (point == null && !isNullOrEmpty(aNumber) && isNullOrEmpty(aBuilding)
                && !isNullOrEmpty(aFlat)) {
            ...
            point = findPoint(aAddress, aNumber, null, aBirthday, null);
            // дом
        } else if (point == null && !isNullOrEmpty(aNumber)) {
    ...
    public String addPatient(String aLastname, String aFirstname, String aMiddlename,
        ...
        VocSocialStatus statusSocial = manager.find(VocSocialStatus.class, aSocialStatus);
        VocSex sex = manager.find(VocSex.class, aSex);
        Date birthday;
    ...
    public String getDoubleByBaseData(String aId, String aLastname, String aFirstname, String aMiddlename,
        ...
        if (!isNullOrEmpty(aPassportType)
                && !isNullOrEmpty(aPassportNumber)
                && !isNullOrEmpty(aPassportSeries)) {
    ...
    public Patient getPatient(String aLastname, String aFirstname, String aMiddlename, Date aBirthDay, String aSexCode, String aCommonNumber
        ...
        List<Patient> patients = manager.createQuery("from Patient where lastname=:lastname and firstname=:firstname" +
        ...
        if (patients.isEmpty()) { //Создаем нового пациента
            ...
            if (addData.has("ogrn")) {
                ...
                try {
                    List<RegInsuranceCompany> regInsuranceCompanies = manager.createQuery("from RegInsuranceCompany where ogrn=:ogrn" +
    ...
    private <T> T getByCode(Class aClass, String aCode, String aFldName) {
        List<T> list = manager.createQuery("from " + aClass.getName() + " where " + (aFldName != null ? aFldName : "code") + "=:code")
