...
public class ContractServiceBean implements IContractService {
	...
	public Long getMedContractBySmo(String aDtypeSmo, Long aIdSmo, boolean aIsRecalc) {
		if (aDtypeSmo==null) return null ;
		Long contract = null ;
		Long patient = null ;
		String date = null ;
		if (aDtypeSmo.equals("HOSPITALMEDCASE")) {
			List<Object[]> l = theManager.createNativeQuery("select sls.patient_id as cpid,mc.id as mcid from medcase sls" 
				...
				+" where sls.id="+aIdSmo+" and mc.id is not null").getResultList();
			if (l.size()==1) {
