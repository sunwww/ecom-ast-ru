...
function printNoActuality(aCtx, aParams) {
    ...
    var sqlReason = "";
    if (+disabilityReason > 0) {
        sqlReason = sqlReason + " and dd.disabilityReason_id=" + disabilityReason;
    }
    if (+closeReason > 0) {
        sqlReason = sqlReason + " and dd.closeReason_id=" + closeReason;
    }
    if (+primarity > 0) {
        sqlReason = sqlReason + " and dd.primarity_id=" + primarity;
    }
    var typeLpu = aParams.get("typeLpu");
    if (+typeLpu == 1) {
        sqlReason = sqlReason + " and dd.anotherLpu_id is null";
    }
    if (+typeLpu == 2) {
        sqlReason = sqlReason + " and dd.anotherLpu_id is not null";
    }
