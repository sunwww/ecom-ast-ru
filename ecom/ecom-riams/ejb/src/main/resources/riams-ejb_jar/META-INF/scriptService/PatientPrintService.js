...
function printCovid(aCtx, aParams) {
    ...
    var list = aCtx.manager.createNativeQuery("select to_char((mc.datestart+1),'dd.mm.yyyy') as dsn1,st.code as stc" +
        ", case when mc.datefinish is null then '' else to_char((mc.datefinish+1),'dd.mm.yyyy') end as df1" +
        ", vbt.code as vbtCode" +
        " from medcase mc" +
        " left join statisticstub st on st.id=mc.statisticstub_id" +
        " left join medcase slo on slo.parent_id = mc.id and slo.dtype='DepartmentMedCase'" +
        " left join bedfund as bf on bf.id=slo.bedfund_id" +
        " left join vocbedtype vbt on vbt.id=bf.bedType_id" +
        " where mc.id=" + hosp.id).getResultList();
    if (!list.isEmpty()) {
        var obj = list.get(0);
        ...
        if (+obj[3] == 14)
            map.put("dopMkb", "Внебольничная двусторонняя вирусная пневмония");
        else
            map.put("dopMkb", "");
    }
