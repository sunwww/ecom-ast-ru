...
function updateDateByMedService(aCtx, aId, aIds, aField, aDate) {
    ...
    if (+prov == 0) {
        if (aField == "start") {
            throw "Дата начала родительской категории должна быть меньше или равна изменяемой даты";
        } else {
            throw "Дата окончания родительской категории должна быть больше или равна изменяемой даты";
        }
    }
    var ids = aIds.split(",");
    ;
