function onPreDelete(aEntityId, aContext) {
	...
	if (patient!=null) {
		...
		+",(select count(*) from DisabilityCase as dc where dc.patient_id=p.id) as v4"
		+",(select count(*) from Kinsman as k1 where k1.person_id=p.id) as v5"
		+",(select count(*) from Kinsman as k2 where k2.kinsman_id=p.id) as v6"
		+",(select count(*) from MedPolicy as pol where pol.patient_id=p.id) as v7"
		+",(select count(*) from Privilege as bc where bc.person_id=p.id) as v8"
		+",(select count(*) from Worker as dc where dc.person_id=p.id) as v9"
		+",(select count(*) from PsychiatricCareCard as pcc where pcc.patient_id=p.id) as v10"
		+",(select count(*) from Pregnancy as pr where pr.patient_id=p.id) as v11"
		+",(select count(*) from ExtDispCard as edc where edc.patient_id=p.id) as v12"
		+",(select count(*) from QualityEstimationCard qec where qec.patient_id=p.id) as v13"
		+",(select count(*) from extdispplanpopulationrecord qec where qec.patient_id=p.id) as v14"
		+",(select count(*) from covid19  qec where qec.patient_id=p.id) as v15"
		+",(select count(*) from contractperson  cp where cp.patient_id=p.id) as v16"
