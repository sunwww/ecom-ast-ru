...
function onPreSave(aForm, aEntity, aCtx) {
    ...
    if (aForm.getEmergency() == null || !aForm.getEmergency()) {
        ...
        if (pat.newborn == null) {
            if (+aForm.orderMkb > 0) {
                ...
                throw "При плановой госпитализации необходимо заполнять поле Код МКБ направителя!!!";
            }
            if (+aForm.orderLpu > 0) {
            } else {
                throw "При плановой госпитализации необходимо заполнять поле Кем направлен!!!";
            }
            if (+aForm.sourceHospType > 0) {
            } else {
                throw "При плановой госпитализации необходимо заполнять поле Тип направившего ЛПУ!!!";
            }
        }
        ...
        if (+aForm.orderType > 0) {
            ...
            throw "При экстренной госпитализации раздел доставлен является обязательным для заполнения!!!";
        }
        if (+aForm.preAdmissionTime > 0) {
        } else {
            throw "При экстренной госпитализации раздел доставлен является обязательным для заполнения!!!";
        }
    }
    if (pat.newborn == null) {
        if (+aForm.orderLpu > 0) {
            if (+aForm.orderMkb == 0 && (aForm.getEmergency() != null && !aForm.getEmergency())) {
                throw "Если указано поле кем направлен необходимо заполнить код МКБ направителя!!!";
            }
        }
    }
