...
<tiles:insert page="/WEB-INF/tiles/printLayout.jsp" flush="true" >
  ...
  <%
    ...
    <msh:sectionContent>
    <ecom:webQuery name="journal_priem" nativeSql=" select p.id as pid,m.number as mnumber, p.lastname||' '|| p.firstname||' '||p.middlename as fiopat,p.birthday,  t.id as tid,t.date as tdate,vwf.name||' '||wp.lastname||' '|| wp.firstname||' '||wp.middlename as wfinfo,mkb.code as mkbcode ,vr.name as vrname  from Ticket t left join medcard m on m.id=t.medcard_id     left join patient p on p.id=m.person_id     left join workfunction wf on wf.id=t.workFunction_id    left join vocworkfunction vwf on vwf.id=wf.workFunction_id    left join worker  w on w.id=wf.worker_id    left join patient wp on wp.id=w.person_id    left join vocIdc10 mkb on mkb.id=t.idc10_id    left join vocreason vr on vr.id=t.vocreason_id    where t.date  between '${param.dateBegin}'  and '${param.dateEnd}' ${add}     order by p.lastname,p.firstname,p.middlename" guid="4a720225-8d94-4b47-bef3-4dbbe79eec74" />
