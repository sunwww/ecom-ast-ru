...
<tiles:insert page="/WEB-INF/tiles/mainLayout.jsp" flush="true" >
  ...
  <msh:section>
    ...
    <msh:sectionContent>
  	<ecom:webQuery name="list" nativeSql="select m.id,ss.code,m.dateStart,m.dateFinish,p.lastname||' '||p.firstname||' '||p.middlename,m.username,d.name as dname,case when m.emergency=1 then 'Да' else 'Нет' end  as memergency,vdh.name as vdhname,m.dateStart,m.dateFinish from MedCase as m left join patient p on p.id=m.patient_id left join vocdeniedhospitalizating vdh on vdh.id = m.deniedhospitalizating_id left join mislpu d on d.id=m.department_id left join StatisticStub ss on ss.id=m.statisticStub_id 
  	where m.DTYPE='HospitalMedCase' and m.dateFinish between cast('${dateBegin }' as Date) and cast('${dateEnd }' as Date) 
