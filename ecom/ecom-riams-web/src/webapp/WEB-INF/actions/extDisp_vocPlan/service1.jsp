...
<tiles:insert page="/WEB-INF/tiles/main${param.short}Layout.jsp" flush="true">
	<tiles:put name="body" type="string">
		...
		for (int i=0;i<cntAgeGroup;i++) {
			...
			ageSql.append(",max(case when edps.plan_id='").append(plan).append("' and edps.ageGroup_id='").append(wqr.get1()).append("' and (edps.sex_id is null or vs.omcCode='1') then 1 else 0 end)") ; 
			ageSql.append("||''||max(case when edps.plan_id='").append(plan).append("' and edps.ageGroup_id=").append(wqr.get1()).append(" and (edps.sex_id is null or vs.omcCode='2') then 1 else 0 end) as SA").append(wqr.get1()).append("G") ;
		}
		StringBuilder sqlResult = new StringBuilder() ;
		sqlResult.append("select veds.id,veds.code,veds.name ") ;
		sqlResult.append(ageSql) ;
...
for (int i=0;i<cntResult;i++) {
	...
	<tr class='dataTr'>
	<%
		out.print("<td rowspan='2'>");out.print(wqr1[0]) ;out.println("</td>");
		out.print("<td rowspan='2'>");
		out.print(wqr1[1]) ;out.println("</td>");
		...
		out.print("<a href='javascript:void(0)' class='adivs' onclick=\"updateExtDispPlanServiceAllAge('");out.print(wqr1[0]);out.print("',this)\">+</a>") ;
		out.print(wqr1[2]) ;out.println("</td>");
		...
		for (int ai=0;ai<cntAgeGroup ;ai++) {
			...
			if (obj!=null&&obj.length()==2) {
				...
				if (obj.substring(0,1).equals("1")) {
					strM.append("<a title='"+ageNameList[ai]+"' class='adivs service"+wqr1[0]+"' href='javascript:void(0)' id='service");
					strM.append(wqr1[0]) ;
				...
				if (obj.substring(1,2).equals("1")) {
					strW.append("<a title='"+ageNameList[ai]+"' class='adivs service"+wqr1[0]+"' href='javascript:void(0)' id='service"+wqr1[0]+"_"+age+"_2' onclick=\"updatePlanService('"+wqr1[0]+"','"+age+"','2')\">+</a>");
				} else {
					strW.append("<a title='"+ageNameList[ai]+"' class='adivs service"+wqr1[0]+"' href='javascript:void(0)' id='service"+wqr1[0]+"_"+age+"_2' onclick=\"updatePlanService('"+wqr1[0]+"','"+age+"','2')\">-</a>");
				}
				strW.append("</td>") ;
