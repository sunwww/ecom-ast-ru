...
wp.lastname,vwf.name,count(distinct caos.id),
...
group by ppG.lpu_id,pp.code,pp.name,wp.lastname,vwf.name
order by pp.name
	...
	<%} else if (typeGroup!=null&& (typeGroup.equals("2") || typeGroup.equals("4"))) {%>
			<msh:section>
			<ecom:webQuery name="finansReport" nameFldSql="finansReport_sql" nativeSql="
select caos.id,p.patientSync,p.lastname,p.firstname,p.middlename,cao.operationdate,
pp.code,pp.name,count(distinct caos.id),
sum(round(cams.countMedService*(cams.cost*(100-coalesce(cao.discount,0))/100),2))
from ContractAccountOperationByService caos
left join ContractAccountOperation cao on caos.accountOperation_id=cao.id and cao.dtype='OperationAccrual' and cao.repealOperation_id is null
left join medcase mc on mc.id=caos.medcase_id
...
left join medservice ms on pms.medservice_id=ms.id
where cao.operationdate
between to_date('31.07.2014','dd.mm.yyyy') and to_date('28.08.2014','dd.mm.yyyy')
and ppG.lpu_id in (184,180 )
and pp.positionType_id=2
...
order by p.lastname,p.firstname,p.middlename,pp.name
 ...
 to_date('${param.dateFrom}', 'dd.mm.yyyy') AND to_date('${param.dateTo}', 'dd.mm.yyyy') 
