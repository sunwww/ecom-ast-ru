 ...
 * @version $Revision: 1.1.2.2 $ $Date: 2005/04/06 20:29:09 $
 */
public class Base64Util {
    ...
    public static String decode(String aStr) {
        if(aStr!=null) {
            try {
                ...
                byte[] bytes = decode(aStr.getBytes("Cp1251")) ;
                return new String(bytes, "Cp1251") ;
            } catch (UnsupportedEncodingException e) {
                throw new IllegalStateException("Ошибка перекодирование строки",e) ;
            }
        } else {
            return null ;
        }
    ...
    public static String encode(String aStr) {
        if(aStr!=null) {
            try {
                byte[] bytes = encode(aStr.getBytes("Cp1251")) ;
                return new String(bytes, "Cp1251") ;
            } catch (UnsupportedEncodingException e) {
                throw new IllegalStateException("Ошибка перекодирование строки",e) ;
            }
...
//  private static final int SIXBIT = 6;
    ...
    static boolean isArrayByteBase64(byte[] arrayOctect) {
        ...
        for (int i = 0; i < length; i++) {
            if (Base64Util.isBase64(arrayOctect[i]) == false) {
                return false;
            }
        }
        return true;
