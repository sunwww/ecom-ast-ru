...
public class UserMessageTag extends SimpleTagSupport {
    ...
    public void doTag() throws JspException, IOException {
        ...
        List<ClaimMessage> claimMessages = ClaimMessage.findInRequest(ctx.getRequest());
        if (claimMessages != null) {
            for (ClaimMessage message : claimMessages) {
        ...
        if (messages != null && !messages.isEmpty()) {
            ...
            for (UserMessage message : messages) {
                sql.append(" jQuery.toast({")
                        ...
                        .append(",heading:'").append(toHtmlString(message.getTitle())).append("'").append(",text:'")
                        .append(toHtmlString(message.getMessage()))
                        .append(!StringUtil.isNullOrEmpty(message.getUrl()) ? "<a href=\"" + message.getUrl() + "\" target=\"_blank\"> Перейти</a>'" : "'")
